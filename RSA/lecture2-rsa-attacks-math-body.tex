...
\medskip\noindent\textbf{One reading tip before we start.} Every attack below has the same three-part shape, and it's worth watching for it explicitly: (1) some \emph{public} quantities are related to the secret by an algebraic identity (an exponent law, a CRT congruence, a Bézout identity); (2) a size or structural assumption (a too-small $d$, an identical broadcast message, a shared modulus) lets that identity be upgraded from ``true modulo something'' to ``true as an ordinary equation''; and (3) that upgraded equation is then solved directly, with ordinary algebra, using no private key. Recognizing this shape in advance makes each individual proof below far easier to follow, because you'll already know what each step is trying to accomplish before you read it.
