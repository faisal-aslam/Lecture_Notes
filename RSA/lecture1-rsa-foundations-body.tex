...
\medskip\noindent\textbf{What CRT says, before any formalism.} Suppose $n=pq$ for two primes sharing no common factor (any two distinct primes qualify). The Chinese Remainder Theorem says: knowing a number $x$'s remainder mod $p$ \emph{and} its remainder mod $q$ is exactly as much information as knowing $x$'s remainder mod $n=pq$ directly — nothing is lost, and nothing is ambiguous. Concretely: every pair of remainders $(a,b)$ with $0\le a<p,\ 0\le b<q$ corresponds to \emph{exactly one} $x\in\{0,\dots,n-1\}$ satisfying $x\equiv a\pmod p$ and $x\equiv b\pmod q$ simultaneously — always solvable, and never more than one solution. This is what later lets us split one hard computation mod $n$ into two easier computations, mod $p$ and mod $q$, and stitch the two answers back together with no loss of information.

\medskip\noindent\textbf{A non-mathematical analogy.} Picture two interlocking gears, with $p$ and $q$ teeth respectively, both starting at a marked tooth and turning together. If $p$ and $q$ share no common factor, the combined configuration (``which tooth of the first gear is marked now'', ``which tooth of the second gear is marked now'') does not repeat until $pq$ steps have passed, and every one of the $pq$ possible combined configurations occurs exactly once per cycle. ``Remainder mod $p$'' and ``remainder mod $q$'' are exactly the two gears' positions; CRT says these two positions, taken together, pin down the combined state uniquely — which is the same information as a single remainder mod $pq$.
...
\medskip\noindent\textbf{Unpacking ``ring isomorphism.''} This is precise language for the informal statement above, plus one extra guarantee. ``Map'' means every $x$ is sent to the pair of its two remainders. ``Isomorphism'' means this map is a perfect one-to-one correspondence (a bijection) between $\{0,\dots,n-1\}$ and all pairs $(a,b)$ with $0\le a<p,\,0\le b<q$ — the ``unique solution'' half of the theorem. ``Ring'' (rather than just ``set'') means the correspondence also respects \emph{arithmetic}: adding (or multiplying) two numbers mod $n$ and then splitting into remainders gives the same pair as splitting first and then adding (or multiplying) each coordinate separately. In symbols, if $x\mapsto(a,b)$ and $y\mapsto(c,d)$, then
\[
x+y \mapsto (a+c \bmod p,\ b+d \bmod q), \qquad xy \mapsto (ac \bmod p,\ bd \bmod q).
\]
This extra guarantee is what makes CRT more than a counting trick: an entire arithmetic computation can be carried out \emph{coordinatewise}, mod $p$ and mod $q$ separately, and the two partial answers automatically recombine into the correct answer mod $n$ — exactly the mechanism Section~\ref{sec:crt-speedup} exploits for faster decryption, and Section~\ref{sec:correctness} exploits for a correctness proof covering every message, not just the coprime ones.

\begin{proof}[Proof, explained step by step]
We check the three things the theorem claims: the map preserves arithmetic, it never sends two different numbers to the same pair, and (by a counting argument) it must therefore hit every pair.

\textbf{It preserves arithmetic (it's a ring homomorphism).} If $x\equiv a\pmod p$ and $y\equiv c\pmod p$, then directly from the definition of congruence, $x+y\equiv a+c\pmod p$ and $xy\equiv ac\pmod p$ — and likewise mod $q$. So reducing mod $p$ (or mod $q$) after adding/multiplying gives the same result as adding/multiplying the already-reduced remainders — exactly the coordinatewise formula displayed above.

\textbf{It is injective (no two different numbers share a remainder pair).} Suppose $x$ and $y$ (both in $\{0,\dots,n-1\}$) map to the \emph{same} pair, i.e.\ $x\equiv y\pmod p$ and $x\equiv y\pmod q$. The first says $p\mid(x-y)$; the second says $q\mid(x-y)$. Since $p,q$ are distinct primes ($\gcd(p,q)=1$), a number divisible by both must be divisible by their product: $pq\mid(x-y)$. But $x,y\in\{0,\dots,n-1\}$ with $n=pq$, so $|x-y|<n=pq$; the only multiple of $pq$ with absolute value less than $pq$ is $0$ itself, forcing $x=y$. So distinct inputs always land on distinct output pairs.

\textbf{It is therefore also surjective (every pair is hit).} The domain $\{0,\dots,n-1\}$ has exactly $n=pq$ elements. The codomain — all pairs $(a,b)$ with $0\le a<p,\,0\le b<q$ — also has exactly $p\cdot q=n$ elements. An injective map between two \emph{finite sets of the same size} must also be surjective (if some pair were missed, the $n$ distinct images would have to fit into fewer than $n$ remaining slots, which is impossible). So every pair $(a,b)$ is the image of exactly one $x$ — precisely the ``unique solution'' claim.
\end{proof}

\medskip\noindent\textbf{A constructive method, not just an existence proof.} The proof above guarantees a unique solution exists but does not by itself hand you a formula for it. In practice — and this is exactly the formula CRT-RSA decryption automates in Section~\ref{sec:crt-speedup} — the solution is built explicitly with the Extended Euclidean Algorithm:
\begin{enumerate}
...
\]
\end{enumerate}
Read informally: start from $x=b$, which already satisfies the mod-$q$ condition; then add a correction term, a multiple of $q$, chosen so the result also satisfies the mod-$p$ condition without disturbing the mod-$q$ one (adding any multiple of $q$ never changes the value mod $q$).

\begin{example}[Solving a CRT system two ways]
Solve $x\equiv2\pmod3$, $x\equiv3\pmod5$ (so $p=3,\,a=2,\,q=5,\,b=3,\,n=15$).

\emph{By listing} (fine for tiny numbers, but does not scale): $x\in\{2,5,8,11,14,\dots\}$ satisfies the first congruence; checking each against the second, $x=8$ gives $8\equiv3\pmod5$. \checkmark So $x\equiv8\pmod{15}$.

\emph{By the constructive formula} (the method that scales, and the one RSA actually uses): compute $q^{-1}\bmod p=5^{-1}\bmod3$. Since $5\equiv2\pmod3$ and $2\cdot2=4\equiv1\pmod3$, we get $5^{-1}\equiv2\pmod3$. Then
\[
x = b + q\cdot\Big(((a-b)\cdot(q^{-1}\bmod p))\bmod p\Big) = 3+5\cdot\big(((2-3)\cdot2)\bmod3\big) = 3+5\cdot\big((-2)\bmod3\big) = 3+5\cdot1 = 8.
\]
Both methods agree: $x=8$, the unique solution in $\{0,\dots,14\}$ guaranteed by the theorem. This same computation — reduce mod $p$, reduce mod $q$, recombine with a modular-inverse correction term — is exactly what Section~\ref{sec:crt-speedup}'s CRT-RSA decryption algorithm performs, with $a,b$ replaced by the two partial decryptions $m_p,m_q$.
\end{example}
