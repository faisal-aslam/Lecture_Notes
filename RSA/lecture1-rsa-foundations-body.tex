...
Two ideas carry the entire RSA construction: \textbf{Euler's totient function} $\varphi(n)$, which counts how much ``room to maneuver'' modular exponentiation has, and \textbf{Euler's theorem}, which says that raising any suitable number to the power $\varphi(n)$ always lands back on $1$. Encryption and decryption in RSA are both just modular exponentiation, so this ``always lands back on $1$'' fact is precisely what lets decryption undo encryption. We build up both ideas slowly, with a fully worked small example at every step, because the rest of the lecture leans on them constantly.
...
\]
i.e.\ $\varphi(n)$ counts how many integers in $\{1,\dots,n\}$ share \emph{no} common factor with $n$ other than $1$.
\end{definition}
...
Think inclusion--exclusion. Among any $p_1$ consecutive integers, exactly one is divisible by $p_1$, so a $\left(1-\frac1{p_1}\right)$ fraction survives once multiples of $p_1$ are discarded. The same holds independently for $p_2,\dots,p_k$. Because the $p_i$ are distinct primes, ``divisible by $p_i$'' and ``divisible by $p_j$'' turn out to behave independently across $\{1,\dots,n\}$ — this independence is itself a consequence of the Chinese Remainder Theorem, introduced in the next subsection — so the surviving fraction after excluding every prime factor is the \emph{product} of the individual surviving fractions, and multiplying by $n$ gives the formula. The RSA-relevant case $\varphi(pq)=(p-1)(q-1)$ says the same thing concretely: from $\{1,\dots,pq\}$, throw away the $q$ multiples of $p$ and the $p$ multiples of $q$ (these two lists overlap only at $pq$ itself, discarded either way), leaving $pq-p-q+1=(p-1)(q-1)$ survivors.
\end{remark}
...
\medskip\noindent\textbf{Why $\varphi(n)$ matters at all.} $\varphi(n)$ is the size of the set of residues that behave ``nicely'' under multiplication mod $n$ — formally, $(\mathbb{Z}/n\mathbb{Z})^{*} = \{a \in \{1,\dots,n-1\} : \gcd(a,n)=1\}$. This set is closed under multiplication mod $n$ (if $a$ and $b$ each share no factor with $n$, multiplying them creates no new common factor with $n$ either), every element has a multiplicative inverse mod $n$ inside the same set (exactly what the Extended Euclidean Algorithm computes), and $1$ acts as the identity. In other words, $(\mathbb{Z}/n\mathbb{Z})^{*}$ is a \textbf{group} of size $\varphi(n)$ under multiplication mod $n$. Euler's theorem, next, is a statement about \emph{this} group.
...
\medskip\noindent\textbf{Plain-English statement, before the proof.} Take any $a$ sharing no factor with $n$, and start computing $a^1, a^2, a^3,\dots$ modulo $n$. Euler's theorem guarantees that by exponent $\varphi(n)$ you have landed back on $1$ — regardless of which coprime $a$ you started with. This ``guaranteed return to $1$'' is the single fact RSA is built on: it is what lets a carefully chosen \emph{second} exponentiation undo the effect of a first one.
...
\textbf{Step 1: multiplying by $a$ just reorders the group.} Consider $ag_1, ag_2, \dots, ag_{\varphi(n)} \pmod n$. Each $ag_i$ is still coprime to $n$ (a product of two things coprime to $n$), so every value in this new list is again some element of $(\mathbb{Z}/n\mathbb{Z})^{*}$. No two of them coincide either: if $ag_i \equiv ag_j \pmod n$, then since $\gcd(a,n)=1$ we may multiply both sides by $a^{-1}\bmod n$ (which exists precisely because $\gcd(a,n)=1$) to get $g_i \equiv g_j \pmod n$, forcing $i=j$. So the list $ag_1,\dots,ag_{\varphi(n)}$ is $\varphi(n)$ \emph{distinct} elements drawn from a set that only has $\varphi(n)$ elements total — meaning it is simply $\{g_1,\dots,g_{\varphi(n)}\}$ written in a different order.
...
\medskip\noindent\textbf{Why RSA cares --- walked through slowly.} RSA encryption raises a message $a$ to a public power $e$; decryption raises the result to a private power $d$. For decryption to recover $a$, we need
\[
(a^e)^d = a^{ed} \equiv a \pmod n.
\]
So the whole question is: \emph{what condition on $e$ and $d$ makes raising to the power $ed$ act like doing nothing at all, modulo $n$?} Euler's theorem answers this directly, and the answer is exactly why RSA key generation is built the way it is. Suppose $e$ and $d$ are chosen so that
\[
...
\]
Chaining the two displayed calculations together gives exactly $a^{ed}\equiv a\pmod n$ — decryption undoes encryption.
...
This is \emph{exactly} the decryption-undoes-encryption identity RSA needs, and Section~\ref{sec:rsa-construction} turns it into an actual key-generation recipe: pick $e$ first, then solve $ed\equiv1\pmod{\varphi(n)}$ for $d$ using the Extended Euclidean Algorithm (precisely how one computes a multiplicative inverse — here, the inverse of $e$ modulo $\varphi(n)$). The one loose thread is the assumption $\gcd(a,n)=1$: a real message $a$ sent through RSA might, in principle, share a factor with $n$. Section~\ref{sec:correctness} closes this gap completely, and it is exactly where the Chinese Remainder Theorem — introduced next — does its first job for us.

\subsection{The Chinese Remainder Theorem}\label{ssec:crt}

\medskip\noindent\textbf{Why we need this at all.} The exponent identity we just proved, $a^{ed}\equiv a\pmod n$, leaned on one assumption: $\gcd(a,n)=1$. A real RSA message $a$ is \emph{almost} always coprime to $n$, but not provably always — so the correctness proof in Section~\ref{sec:correctness} has a gap to close. CRT is the tool that closes it, and it will also give us a $4\times$ decryption speedup later (Section~\ref{sec:crt-speedup}). Before any of that, let's just understand what CRT \emph{says}, with numbers, before touching the formal statement.
...
\medskip\noindent\textbf{The critical logical relationship.} \[
