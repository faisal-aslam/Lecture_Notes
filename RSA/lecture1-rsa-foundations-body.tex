...
\medskip\noindent\textbf{Why RSA cares.} Euler's theorem tells us: if we pick an exponent $e$ and a matching $d$ such that $ed \equiv 1 \pmod{\varphi(n)}$, then for any $a$ coprime to $n$,
\[
...
\item \textbf{Efficiency}: it lets us speed up decryption by roughly $4\times$ using CRT-RSA (Section~\ref{sec:crt-speedup}) — and this optimization is precisely what makes the Bellcore fault attack possible, which we cover in Lesson 3.
\end{enumerate}
\end{remark}
