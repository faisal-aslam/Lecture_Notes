...
\medskip\noindent\textbf{Why this lecture feels different from the last two.} In Lessons 1--2, every attack lived entirely on paper: given some numbers, do some algebra, get the secret. Here, the ``numbers'' are produced by a real physical computer running real code, and that computer takes measurable time, draws measurable power, and can be made to make mistakes (deliberately, by an attacker). The mathematics you already know (CRT, Euler's theorem, Coppersmith's method) does not change — it gets \emph{reused}, but now the input to that mathematics is something the attacker extracted from the outside world rather than something sitting in a public key file. Keep asking, for each attack below: \emph{what physical signal is being turned into a piece of secret information, and by which piece of Lesson 1--2 mathematics is that piece then exploited?}
...
\medskip\noindent\textbf{Why one bit is catastrophic — the mechanism, unpacked.} A one-bit oracle sounds harmless — surely one bit of information about a giant secret is nothing? Bleichenbacher (CRYPTO 1998) showed the opposite, and the mechanism has two ingredients worth separating clearly.

\textbf{Ingredient 1: RSA lets you scale the hidden message by a chosen factor, without knowing it.} Recall RSA's malleability from Lesson 1: given $c=m^e\bmod n$, anyone can compute $c\cdot s^e\bmod n$ for a chosen public $s$, and this is a valid encryption of $m\cdot s\bmod n$ — because $c\cdot s^e \equiv m^e s^e \equiv (ms)^e\pmod n$, pure exponent-law algebra, no secret required. So an attacker who only ever gets a single ciphertext $c$ can still generate an unlimited family of \emph{related} ciphertexts $c\cdot s^e\bmod n$, one for every $s$ they choose, each corresponding to the unknown $m$ scaled by that chosen $s$.

\textbf{Ingredient 2: the oracle turns ``is $ms\bmod n$ in a certain narrow range'' into a one-bit test.} The padding format requires the decrypted block to start with \texttt{0x00 0x02}, which — read as a number — means the decrypted value must fall in a specific narrow range $[2B,3B)$ out of all of $\{0,\dots,n-1\}$, where $B=2^{8(k-2)}$. So submitting $c\cdot s^e\bmod n$ to the oracle and getting a YES answer tells the attacker, for free: \emph{the unknown quantity $m\cdot s\bmod n$ currently lies inside $[2B,3B)$} — a strong constraint on $m$, since $s$ is chosen and known.

\textbf{Putting the two together.} Each oracle query is a free membership test — ``does $m$ times my chosen $s$ land in this known narrow band?'' — and each YES answer eliminates most of the remaining candidates for $m$. Repeating this adaptively, choosing each new $s$ based on everything learned so far, behaves like a binary search over the space of possible $m$: not quite halving the candidate set every query (the geometry is messier than a clean bisection), but shrinking it steadily and predictably until only one value of $m$ survives. This is a classic \textbf{adaptive chosen-ciphertext attack}: it uses the decryption oracle itself, not any weakness in the RSA problem.
...
\medskip\noindent\textbf{Reading the algorithm against the two ingredients above.} At every stage, the attacker maintains a set of intervals guaranteed to contain the true $m$ (initially, the whole range $\{0,\dots,n-1\}$). Each new query picks a candidate $s_i$ (Ingredient 1: scale by a chosen, known factor) and asks the oracle whether $m s_i \bmod n$ lands in $[2B,3B)$ (Ingredient 2: the one-bit membership test). A YES answer means: for whichever interval $m$ was in, only the sub-range consistent with $ms_i\in[2B,3B)$ survives — everything else is provably ruled out and discarded. Repeating this steadily shrinks the total surviving range, exactly like narrowing down a hidden number through a sequence of ``is it bigger than / smaller than'' questions, until only a single value of $m$ remains consistent with every answer received so far.
...
\medskip\noindent\textbf{Why this works even though a single timing measurement is far too noisy to read a bit off directly.} A single decryption's total time $T_i$ is the sum of contributions from \emph{every} step of the exponentiation — thousands of squarings and conditional multiplications — plus random measurement noise (network jitter, OS scheduling, cache state left over from other processes). No single measurement isolates the cost of one specific bit's decision. The trick is to use \emph{many} ciphertexts and \emph{group them by prediction}: for a guessed value of the next bit, simulate whether the extra multiplication should occur for each $c_i$, and split the sample into two groups — ``simulation predicts extra multiply'' versus ``simulation predicts no extra multiply.'' If the guessed bit is correct, the first group's real measured times $T_i$ will be, \emph{on average}, very slightly higher than the second group's — because they really did perform one more multiplication each — and this tiny average difference becomes statistically detectable once enough samples are averaged together, the same way a coin's small bias becomes obvious after enough flips even though any single flip tells you nothing. If the guessed bit is wrong, the simulation's grouping is uncorrelated with what the hardware actually did, so both groups' average times come out statistically indistinguishable. This is precisely why the attack proceeds strictly one bit at a time, most-significant first: correctly recovering bit $j$ requires the simulation of steps up to bit $j$ to already be accurate, which needs every higher-order bit to already be known.
...
\medskip\noindent\textbf{Why this should feel dangerous, before the proof.} Recall from Lesson 1 exactly what CRT guarantees: the map $x\mapsto(x\bmod p,\ x\bmod q)$ is a perfect one-to-one correspondence — every pair of remainders corresponds to \emph{exactly one} $x$ modulo $n$. CRT-RSA signing builds a value in exactly the reverse direction: it computes two remainders ($s_p$, $s_q$) separately and \emph{trusts} that recombining them gives the one true $x$ that has those two remainders. That trust is precisely what a fault attack violates. If a hardware glitch corrupts only $s_p$, the recombination step goes on trusting it anyway and confidently reconstructs \emph{a} number with the given pair of remainders — just not the number anyone intended. The faulty output is CRT working perfectly on a garbage input, and the very same reconstruction formula that recombines correct partial results also, unknowingly, discloses exactly where the two halves disagree.
...
\textbf{Step 1: pin down what the fault does and does not change.} The $q$-branch computation was untouched by the fault, so the faulty output $s'$ still agrees with the correct signature modulo $q$: $s' \equiv m^d \pmod q$. The $p$-branch was corrupted, so $s'$ disagrees with the correct signature modulo $p$: $s' \not\equiv m^d \pmod p$ (a random hardware fault essentially never happens to land on a value that accidentally still satisfies the same congruence).

\textbf{Step 2: translate ``agrees mod $q$, disagrees mod $p$'' into a divisibility statement about $s'^e - m$.} Correctness of RSA (Lesson 1, Section~\ref{sec:correctness}) says that for the \emph{true} signature $s=m^d$, we'd have $s^e\equiv m\pmod n$, and in particular $s^e\equiv m\pmod q$ and $s^e\equiv m\pmod p$ individually. Since $s'\equiv m^d\pmod q$ still holds (Step 1), the same correctness argument restricted to mod $q$ gives $s'^e\equiv m\pmod q$, i.e.
\[
q \mid (s'^e - m).
\]
But $s'\not\equiv m^d\pmod p$, so the parallel argument mod $p$ \emph{fails}: $s'^e\not\equiv m\pmod p$, i.e.
\[
p \nmid (s'^e - m).
\]

\textbf{Step 3: extract $q$ with a single gcd.} Step 2 says $q$ divides $s'^e-m$ while $p$ does not — exactly the situation where computing $\gcd(s'^e-m\bmod n,\ n)$ isolates $q$ cleanly: any common factor of $s'^e-m$ and $n=pq$ can only involve $q$ (since $p$ does not divide $s'^e-m$ at all), and $q$ itself certainly does divide both. So
\[
\gcd\big(s'^e - m \bmod n,\ n\big) = q.
\]

\textbf{Step 4: finish factoring and rebuild the private key.} With $q$ in hand, compute $p=n/q$ by ordinary integer division, then $\varphi(n)=(p-1)(q-1)$, then $d=e^{-1}\bmod\varphi(n)$ exactly as in ordinary key generation (Lesson 1) — the entire private key falls out from one faulty signature and one gcd computation.
\end{proof}

\medskip\noindent\textbf{The one-sentence summary.} A fault attack on CRT-RSA is CRT's own uniqueness guarantee turned into a weapon: corrupting exactly one branch produces a number that is quietly ``correct modulo $q$, wrong modulo $p$'' rather than uniformly wrong, and that asymmetry is precisely what a single $\gcd$ computation detects and exploits.
...
\medskip\noindent\textbf{Why this is a coset, and why a coset is much smaller than it looks.} Fix $M$ and think of all integers as sorted into $M$ buckets by their remainder mod $M$ — bucket $0$, bucket $1$, \dots, bucket $M-1$. An ordinary randomly-generated prime $p$ could fall in essentially any bucket (any residue coprime to $M$). The vulnerable construction instead \emph{forces} $p$'s bucket to be one of the specific values $65537^a\bmod M$ for some integer $a$ — and because $65537$ has some finite multiplicative order modulo $M$ (Lesson 1's group-order idea, applied to the group $(\mathbb{Z}/M\mathbb{Z})^{*}$), the set of values $65537^a\bmod M$ reachable as $a$ ranges over all integers is not all of $\{0,\dots,M-1\}$ — it cycles through only $\mathrm{ord}_M(65537)$ distinct residues before repeating. So instead of $p$'s bucket being one of $M$ possibilities, it is secretly one of only $\mathrm{ord}_M(65537)$ possibilities — a tiny, explicitly listable subset, even though $M$ itself is astronomically large.
...
\medskip\noindent\textbf{Why Coppersmith's method specifically is the right tool for Step 2.} Fixing a candidate residue $r_a$ turns ``find $p$'' into ``find the unknown $k$ in $p=kM+r_a$'' — and if $p$ has, say, $1024$ bits while $M$ has around $200$ bits, then $k$ has only around $1024-200=824$ bits: a large fraction of $p$ (the residue $r_a\bmod M$) is already pinned down, leaving only a comparatively \emph{small} unknown $k$ to find. This is exactly the ``small root of a polynomial mod $n$'' shape Section~\ref{sec:coppersmith} in Lesson 2 was built for: the polynomial $f(x)=kM+r_a-x$ (thinking of $x$ as standing in for the unknown residual bits) has a small root precisely when the coset construction has pinned down enough of $p$'s bits, which ROCA's fingerprint structure guarantees.
...
\medskip\noindent\textbf{The design goal, before the formulas.} Every attack in Lessons 2--3 up to this point succeeded because \emph{some} structure survived from the plaintext into what actually got RSA-encrypted: a small message, a predictable format, a known relationship between two messages, a cheaply-checkable validity condition. OAEP's goal is to erase all of that: take the message and a fresh random seed, and mix them together so thoroughly that the result is indistinguishable from a uniformly random string to anyone who does not know the seed — while still being perfectly and uniquely invertible by someone who does.
...
\medskip\noindent\textbf{Why two rounds of masking, and why in this order.} A single XOR mask ($X=m\oplus G(r)$ alone) would already hide $m$ from anyone who does not know $r$ — but the seed $r$ itself would then have to travel alongside $X$ in the clear for the recipient to reconstruct anything, defeating the purpose entirely. OAEP's second step, $Y=r\oplus H(X)$, hides $r$ \emph{using $X$ itself as the mask key} — so the seed is never exposed either. This creates a tight two-way dependency: $Y$ cannot be computed without already knowing $X$, and decoding $X$ needs $r$, which needs $Y$ and $X$ together. That interlocking is exactly what makes a single flipped bit anywhere in $\text{EM}$ cascade into an unpredictable, unrelated-looking recovered message on decryption (rather than a small, predictable change) — precisely the property that shuts down chosen-ciphertext manipulation.
...
\medskip\noindent\textbf{Security sketch, in the random oracle model.} Modeling $G$ and $H$ as independent random oracles, Bellare and Rogaway showed OAEP-RSA achieves \textbf{IND-CCA2 security} (indistinguishability under adaptive chosen-ciphertext attack) whenever the underlying trapdoor permutation (here, RSA) is one-way.

\medskip\noindent\textbf{The proof idea, unpacked.} ``$G,H$ are random oracles'' means: the only way to learn anything about the output of $G$ or $H$ on a given input is to explicitly query them on that exact input — there is no shortcut, no formula to exploit, only lookups. Suppose an adversary could distinguish OAEP encryptions of two messages, or extract useful information from a decryption-oracle query on a chosen ciphertext. Either capability requires the adversary to have gotten \emph{some} information correlated with the true seed $r$ or the true masked value $X$ underlying a target ciphertext $c=\text{EM}^e\bmod n$ — and because $G,H$ are random oracles, the \emph{only} way to obtain such correlated information is to have already queried $G$ or $H$ on that specific $r$ or $X$. But querying $G$ or $H$ on the value tied to $c$'s RSA pre-image is, itself, morally equivalent to already knowing $\text{EM}=c^d\bmod n$ — i.e., to already having inverted RSA on $c$. So a simulator who watches the adversary's oracle queries can extract an RSA inverse from whichever query turns out to be the ``real'' one. Informally: \textbf{any successful attack on OAEP-RSA can be converted into an algorithm that inverts plain RSA}, which we assume is hard.
...
\medskip\noindent\textbf{What a protocol like this has to achieve, in plain terms, before any algebra.} Think of three separate promises a scheme like this must keep simultaneously, each of which fails for a naive approach:
\begin{itemize}
\item \textbf{Completeness}: someone who genuinely knows the secret should always be able to convince the verifier. (Trivial to achieve alone — just reveal the secret directly.)
\item \textbf{Soundness}: someone who does \emph{not} know the secret should have essentially no chance of convincing the verifier. (Also easy alone — just never let anyone in without proof.)
\item \textbf{Zero-knowledge}: the verifier should learn \emph{nothing} about the secret itself from watching the proof, other than the one bit ``this person knows it.'' (This is the hard one — the whole reason a protocol is needed rather than simply typing in a password, which \emph{does} reveal the secret to whoever is listening.)
\end{itemize}
The tension is between soundness and zero-knowledge: soundness wants the prover to demonstrate real, specific knowledge of $s$, while zero-knowledge wants that demonstration to leak nothing reusable. Guillou--Quisquater resolves this tension with a three-message \textbf{commit/challenge/response} structure: the prover commits to something \emph{before} seeing what will be asked, is then asked an unpredictable question, and answers in a way that only someone who really knows $s$ could answer correctly for an unpredictable question — without the answer itself ever containing $s$.
...
\medskip\noindent\textbf{Reading the protocol against the three goals above.} The commitment $x=r^v\bmod n$ is sent \emph{before} the challenge $c$ is even chosen — the prover cannot tailor $x$ to a challenge they have not seen yet. The response $y=r\cdot s^c\bmod n$ blends the fresh randomness $r$ with the secret $s$ raised to the (unpredictable, just-revealed) challenge power — a genuine one-time mixture, not a static reveal of $s$. The verification equation checks that the blend is consistent with \emph{some} valid $s$, without ever recomputing or isolating $s$ itself.
...
\medskip\noindent\textbf{Why soundness needs the commitment to come first — the intuition behind Proposition~\ref{prop:gq-soundness}.} If the prover could see the challenge $c$ before committing to $x$, cheating would be trivial: fix $y$ arbitrarily, compute $x=y^vJ^c\bmod n$ working backwards, and the check passes for that known $c$ every time, with no knowledge of $s$ required at all. The entire soundness guarantee rests on \emph{ordering}: $x$ is locked in first, $c$ is only then revealed at random, and a fixed $x$ algebraically corresponds to a valid response $y$ for only \emph{one} value of $c$ (since $y^v$ is a fixed function of $x$ and $c$ together, and $v$-th roots mod $n$ are unique once they exist) — so a prover who guessed wrong before committing is simply stuck.

\begin{remark}[Zero-knowledge, sketch]
A simulator that does not know $s$ can still produce transcripts $(x,c,y)$ indistinguishable from real ones \emph{without} interacting with a real prover: pick $c$ and $y$ uniformly at random first, then set $x = y^v J^c \bmod n$ (exactly the ``rewind and fake the commitment'' trick used in Proposition~\ref{prop:gq-soundness}, but now run by the simulator rather than an attacker). Since $x$ is computed from uniformly random $y,c$, the resulting triple has exactly the same distribution as a genuine protocol run, which is the essence of the honest-verifier zero-knowledge argument: nothing in the transcript could not have been produced without knowledge of $s$, so the transcript itself leaks no information about $s$.
\end{remark}

\medskip\noindent\textbf{Why the very same trick proves both soundness \emph{and} zero-knowledge, and why that is not a contradiction.} It looks paradoxical that ``work backwards from $(y,c)$ to fake $x$'' is simultaneously the cheating prover's only strategy (Proposition~\ref{prop:gq-soundness}) and the simulator's entire proof technique (the remark above) — but the two use it in opposite roles. The cheating prover must guess $c$ \emph{before} the real verifier reveals it, and fails whenever the guess is wrong: this is what makes the protocol sound against a real, interactive verifier. The simulator, by contrast, is not trying to fool a live verifier in real time at all — it is only trying to produce a transcript that \emph{looks} like a valid run when inspected afterward, and it is allowed to choose $c$ itself as part of that simulated transcript. The same algebraic backwards-construction is a dead end for real-time cheating but a complete proof technique for showing that transcripts alone contain no extractable information about $s$.
