...
\end{enumerate}
Today makes ``pattern'' from Lecture~\ref{lec:3} completely precise. Later lectures show
\emph{how} a machine matches a regular expression, via finite automata.
\end{coreidea}
...
\label{sec:l4-operations-on-languages}
...
This is exactly $L(L\cup D)^{*}$ from the worked example in \Sref{sec:l4-operations-on-languages},
...
lexer-generator tools like Flex implement only \emph{true} regular expressions, compiled to
...
(via Thompson's construction), covered later in the course. There is a completely different,
more recent technique worth knowing about: the \textbf{derivative} of a regular expression $r$
with respect to a symbol $a$, written $\partial_a(r)$, is a new regular expression describing
``what still needs to match after consuming an $a$.'' Repeatedly taking derivatives, one input
symbol at a time, and checking whether the final derivative accepts $\varepsilon$, matches a
string \emph{without ever building an explicit automaton at all}. Janusz Brzozowski introduced
this in 1964; it fell out of fashion for decades, then saw a resurgence starting around 2010 in
functional-programming circles (Scala's and Racket's parser-combinator libraries use
derivative-based matching), because derivatives compose beautifully with recursive data types
in a way that hand-built automata do not.
\end{enrichment}
...
integration with symbol-table actions -- built to compile to a real DFA. \\
\bottomrule
\end{tabularx}
\end{center}
