...
\begin{tikzpicture}[
  ...
  \node[fe, below=of sem] (icg) {Intermediate Code Generation};
...
\begin{itemize}
  \item \textbf{Lexical analysis.} The character stream is grouped into tokens:
  ...
  \item \textbf{Syntax analysis.} The token stream is organized into a syntax
  ...
  \item \textbf{Semantic analysis.} Types are checked, and where necessary,
  ...
  \item \textbf{Intermediate code generation.} The annotated tree is
...
id1 = t3
\end{verbatim}
  \item \textbf{Machine-independent code optimization.} \texttt{inttoreal(60)} does not depend on
...
id1 = id2 + t1
\end{verbatim}
  \item \textbf{Code generation.} The optimized IR becomes target assembly, using
...
\begin{tikzpicture}[
  ...
  \foreach \p in {lex, syn, sem}
    \draw[-{Latex[length=2mm]}, gray, dashed] (\p.east) -- (eh.west |- \p.east);
\end{tikzpicture}}
\end{center}
\begin{center}
\small\textit{The symbol table is read and written by every phase, but only lexical, syntax,
and semantic analysis report to the error handler -- those are the three error categories this
course teaches (\Sref{sec:l2-error-kinds}). Intermediate code generation and code generation
run only once the tree is already fully checked and error-free, so they are not shown wired to
...
\midrule
\textbf{Front-end} & Lexical, syntax, and semantic analysis, plus intermediate code
generation. Language-\emph{specific}: knows the grammar and type rules of exactly one source
language, and lowers it to a common IR. \\
