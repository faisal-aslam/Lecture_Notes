...
relative to run time} (Lecture 1's AOT/interpreter/JIT distinction). LLVM IR and WebAssembly are
...
2020, first described publicly in~[MLIR]. It is worth understanding alongside LLVM IR for the same reason the
...
close to assembly, as \S5 described. That is exactly what makes it a good shared target for many
...
optimizer passes and back-ends already described in \S3 and \S5. MLIR extends the \emph{front}
...
MLIR dialect called FIR before reaching LLVM IR.

\subsection*{References for This Section}
\begin{itemize}
  \item[{[MLIR]}] Lattner, C., Amini, M., Bondhugula, U., Cohen, A., Davis, A., Pienaar, J.,
  Riddle, R., Shpeisman, T., Vasilache, N., Zinenko, O. \textit{MLIR: A Compiler Infrastructure
  for the End of Moore's Law}. arXiv:2002.11054, 2020.
  \item[{[MLIR-DOCS]}] The MLIR Project. Official documentation. \url{https://mlir.llvm.org/}
\end{itemize}
...
\begin{enumerate}
  ...
  \item For the \texttt{position} example in \S1.1, write out what the three-address code would
