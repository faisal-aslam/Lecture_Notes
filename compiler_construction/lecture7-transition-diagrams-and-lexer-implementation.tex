...
\begin{tikzpicture}[node distance=2.6cm, >=Latex]
  ...
  \draw[trans, ->] (s8) to[out=45, in=135, looseness=6] node[above, font=\small]{letter, digit} (s8);
...
\begin{tikzpicture}[node distance=2.6cm, >=Latex]
  ...
  \draw[trans, ->] (-1.3,0) -- (w0);
  \draw[trans, ->] (w0) to[out=45, in=135, looseness=6] node[above, font=\small, align=center]{blank, tab,\\newline} (w0);
...
\begin{examfocus}
Retraction is also the cleanest illustration of \Sref{sec:l6-lexgen-buffer-cycle}'s safety
condition for reusing a buffer. Every retraction in this lecture steps \texttt{forward} back by
exactly \emph{one} character, so \texttt{lexemeBegin} is never more than one character behind
the last-read position -- meaning it can never sit further back than the buffer immediately
before the one \texttt{forward} currently occupies. That is exactly why it is always safe to
have already overwritten anything two or more buffers back by the time a retraction happens.
\end{examfocus}
