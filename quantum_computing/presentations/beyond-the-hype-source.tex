...
\begin{frame}{Why this talk exists}
  \begin{itemize}
    ...
    \item Two questions I keep hearing on this campus:
    \begin{itemize}
      \item[--] \emph{``Is this real science or Silicon-Valley marketing?''}
      \item[--] \emph{``Should we spend curriculum time and money on it?''}
    \end{itemize}
    \item My claim: \textbf{both extremes are wrong} --- it's neither magic
          nor a mirage
    \item Format: \textbf{Myth} $\rightarrow$ \textbf{Fact}, each one taken apart
          technically but explained so \emph{everyone} in the room follows it
  \end{itemize}
  \note{
    Set the stakes explicitly. Mention that by the end you'll return to the
    curriculum question with concrete data (workforce numbers, market growth,
    what a course would actually look like) so this doesn't feel like a
    detour from the administration's real question. Consider asking a show
    of hands: who has used Qiskit / IBM Quantum Experience? Who thinks
    quantum computers already exist? Who thinks they can break all encryption
    today? Use the answers to calibrate pacing.
  }
\end{frame}
...
\begin{frame}{Roadmap}
  \begin{enumerate}
    ...
    \item The case for \textbf{teaching this now} (10 min)
    \item Curriculum sketch, resources, Q\&A (10 min)
  \end{enumerate}
  \note{
    Roughly 90 minutes total as outlined; trim section 3 or 4 if running at
    the 60-minute end, expand Q\&A and section 6 if the audience skews
    administration-heavy. Section 6 is the payload for the deans in the room
    -- do not let it get squeezed by running long earlier.
  }
\end{frame}
...
\begin{frame}{Physical qubits vs.\ logical qubits}
  ...
  \note{
    ...
    articles they'll read afterward.
  }
\end{frame}
...
\begin{frame}{\myth{Quantum computers will replace classical computing entirely}}
  \fact{Quantum advantage is \textbf{narrow and structural}, not general}
  \begin{itemize}
...
\begin{frame}{\myth{Quantum computers try every possible answer simultaneously, ``by magic''}}
  ...
  \begin{itemize}
    \item Superposition alone gives you \emph{access} to all $2^n$ basis
          states -- but a single measurement reveals only \textbf{one},
          chosen at random by the amplitudes
    \item The actual trick: design a circuit so that amplitudes leading to
          \textbf{wrong} answers cancel out (destructive interference), and
  ...
  \note{
    ...
    which is obviously false (see previous section). The actual source of
    power is designing constructive/destructive interference so that only
    the useful answers survive to measurement time -- this is genuinely
    hard algorithm design, not a free lunch from parallelism. This is why
    only a handful of quantum algorithms exist after 30 years, despite full
    access to superposition being "free."
  }
\end{frame}
...
\begin{frame}{\myth{$n$ qubits store $2^n$ classical bits of information}}
  ...
  \end{itemize}
  \note{
    This myth comes from confusing the size of the MATHEMATICAL DESCRIPTION
...
\begin{frame}{\myth{Entanglement lets you communicate faster than light}}
  \fact{The \textbf{no-communication theorem}: correlation $\neq$ signal}
  \begin{itemize}
...
\begin{frame}{This is not a niche research topic anymore}
  \begin{columns}
  ...
  \note{
    ...
    reusing this deck a year from now. Also mention if useful: quantum
    job/internship postings grew about 11\% in the most recent year measured,
    and separate industry analyses report postings up roughly 180-450\% over
    the last several years depending on methodology -- the wide range itself
    is worth being upfront about, since it reflects how new and unsettled
    the measurement is, not that the growth isn't real.
  }
\end{frame}

\begin{frame}{The skills gap is the argument}
  \begin{itemize}
    \item Analysis of thousands of quantum job postings: roughly
    ...
    \item Average U.S.\ quantum-computing salary already above
          \textbf{\$180,000}; senior roles regularly exceed \$200,000
  ...
  \note{
    ...
    arguments you can make -- unlike, say, standing up a new wet-lab or
    fabrication course, a quantum computing course can start with free or
    low-cost cloud access to actual quantum hardware (IBM's Qiskit platform
    offers free-tier access), open-source software (Qiskit, Cirq, PennyLane),
    and existing physics/CS faculty co-teaching. The skills-gap statistic is
    from EPJ Quantum Technology's analysis of ~3,600 job postings -- cite
    that provenance if pressed on the number. Frame the ask explicitly: this
    is not "should we build a quantum lab," it's "should we teach the
    software/theory layer that students can practice on real cloud hardware
    for free," which is a much smaller institutional lift.
  }
\end{frame}
...
\begin{frame}{What a first course could look like}
  ...
  \end{enumerate}
  \begin{center}\small
  \textit{Prerequisite: linear algebra + intro programming --- accessible to
  juniors, not just PhD students}
  \end{center}
  \note{
    Keep this concrete and modest in scope -- a single semester, 3-credit
    elective, cross-listed between CS and Physics departments is a realistic
    and low-risk first step, not a whole new degree program. Mention that
    many peer universities already run exactly this kind of one-semester
    "Introduction to Quantum Computing" elective with these prerequisites --
    this is a well-trodden path, not an experiment. If asked "who teaches
    it," note that existing CS theory faculty (complexity/algorithms) and
    physics faculty (quantum mechanics) can co-teach with a semester of prep
    -- it does not necessarily require a brand-new hire on day one, which
    lowers the barrier to a pilot offering further.
  }
\end{frame}
...
\begin{frame}{Resources to start today}
  \begin{itemize}
    ...
    \item Student clubs: \textbf{IBM Quantum Educators}, \textbf{QWorld},
          hackathons (e.g.\ \textbf{iQuHACK})
  \end{itemize}
  \note{
    ...
    after a talk like this. If time allows, consider live-demoing a 3-line
    Qiskit circuit on the free IBM Quantum cloud during Q\&A to make the
    "no lab purchase required" point viscerally real.
  }
\end{frame}

% ============================================================
\begin{frame}[standout]
  \Large\textbf{Not magic. Not hype. Real physics, real limits, real jobs.}
  \vspace{4mm}

  \normalsize Questions?
  \note{
    Closing line should land the whole talk's thesis in one sentence. Then
    open the floor. Anticipate: (1) "when will it actually be useful?" --
    answer with the quantum-advantage timeline language from earlier
    (IBM targeting verified advantage by end of 2026, broader
    fault-tolerant systems widely estimated 2030s), stated as industry
    projection, not certainty; (2) "should students specialize in this now
    or is it too early?" -- answer: learn the FUNDAMENTALS (linear algebra,
    complexity theory) which are useful regardless of which hardware
    approach wins, rather than over-betting on one vendor's stack;
    (3) pushback from a skeptical faculty member about hype -- validate the
    skepticism (it's earned, the field has overclaimed before) while
    pointing back at the recap slide as the actual evidence-based position
    you're arguing for.
  }
\end{frame}
