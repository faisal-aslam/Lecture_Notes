...
Let $\{ \ket{\phi_0}, \ket{\phi_1} \}$ be an orthonormal basis for a single-qubit Hilbert space:
\[
\braket{\phi_i | \phi_j} = \delta_{ij}, \quad \sum_{i=0}^1 \ket{\phi_i}\bra{\phi_i} = I.
\]

A \textbf{projective measurement} in this basis is described by the projectors:
\[
P_0 = \ket{\phi_0}\bra{\phi_0}, \qquad
P_1 = \ket{\phi_1}\bra{\phi_1}.
\]

These satisfy the completeness relation: $P_0 + P_1 = I$.

\subsection{The Born Rule Generalized}

For a quantum state $\ket{\psi} = \alpha\ket{0} + \beta\ket{1}$ with $|\alpha|^2 + |\beta|^2 = 1$:

The probability of obtaining outcome $i$ (corresponding to $\ket{\phi_i}$) is:
\[
...
If outcome $i$ is obtained, the state \textbf{collapses} (or updates) to:
\[
\ket{\psi_i} = \frac{P_i \ket{\psi}}{\sqrt{p(i)}} = \ket{\phi_i}.
\]
Note that after measurement in basis $\{\ket{\phi_0}, \ket{\phi_1}\}$, the state is exactly $\ket{\phi_i}$ regardless of the initial state (up to a global phase).

\begin{tcolorbox}[title=Important Distinction]
\textbf{Computational basis measurement:} Projects onto $\ket{0}$ or $\ket{1}$ \\
