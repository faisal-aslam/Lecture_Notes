...
\subsection{Why entropy is a speed limit}

Shannon's \textbf{source coding theorem} states that no lossless coder can, on average, use fewer bits per symbol than the source's entropy. This is the single most important fact in this entire field: \emph{entropy is the floor}. The history of entropy coding (Section~4) is the history of getting closer to that floor while paying less and less in computation time to do it.
...
\begin{center}
\resizebox{\linewidth}{!}{$
\text{raw data} \;\xrightarrow{\text{BWT}}\; \text{permuted data} \;\xrightarrow{\text{MTF}}\; \text{mostly small numbers} \;\xrightarrow{\text{RLE}}\; \text{short runs} \;\xrightarrow{\text{Huffman}}\; \text{compressed output}
$}
\end{center}
