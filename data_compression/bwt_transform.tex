...
\subsection*{Step 1: Understanding the Goal}
...
\subsection*{Step 3: Compute the First Column F}
...
\subsection*{Step 4: Count Occurrences in L (Build Rank Array)}
...
\subsection*{Step 5: Count Occurrences in F}
...
\subsection*{Step 6: Build Cumulative Starting Positions for F}
...
\subsection*{Step 7: The Key Formula}
...
Why (r - 1)? Because:
\begin{itemize}
    \item The first occurrence (r=1) goes to cum[c] + 0
    \item The second occurrence (r=2) goes to cum[c] + 1
    \item The third occurrence (r=3) goes to cum[c] + 2
\end{itemize}
...
\subsection*{Step 8: Compute LF for Each Position}
...
\subsection*{Step 9: Understanding Each Result}

Let's verify each mapping to build intuition:
...
\subsection*{Step 10: Final LF-Mapping Array}
...
\State \(n \gets \text{length}(L)\)
\State Compute LF-mapping array \(LF[0..n-1]\) using Algorithm 8.2
