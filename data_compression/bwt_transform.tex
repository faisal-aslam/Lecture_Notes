...
You might wonder: *BWT already creates runs like \texttt{nn} and \texttt{aa}. Why not just apply RLE directly?*

The answer: BWT creates **clusters**, not necessarily runs. Look at a realistic BWT output:

\[
\text{BWT("mississippi")} = \texttt{ipssm\$pissii}
\]

The four 'i's are scattered. Direct RLE would see mostly isolated characters — poor compression.

\begin{importantbox}[The Insight]
BWT groups similar characters **near** each other, but not necessarily **consecutively**. 

MTF converts this "nearness" into actual runs: when a character repeats (even with small gaps), MTF outputs zeros. The result becomes dominated by long runs of zeros — perfect for RLE.

\textbf{Analogy:} BWT moves similar items into the same neighborhood. MTF walks through and shouts "same as before!" (0) each time it sees the same item again. RLE then compresses those shouts.
\end{importantbox}

Without MTF, BWT output is not directly RLE-friendly. MTF is the essential bridge.
