...
\For{\(i \gets 0\) to \(n-1\)}
    \State \(c \gets L[i]\)
    \State \(rank[i] \gets \text{count of } c \text{ seen so far in } L\)
    \State Increment \(C[c]\)
\EndFor
...
\begin{examplebox}[Computing LF-Mapping for ``annb\$aa'']
Let's compute the LF-mapping for our example step by step.
...
Now, compute cumulative counts for each character in \(F\):

\begin{center}
\begin{tabular}{c|c|c|c}
\textbf{Character} & \textbf{Count in \(F\)} & \textbf{Cumulative} & \textbf{Positions} \\
\hline
\$ & 1 & 0 & 0 \\
a & 3 & 1 & 1,2,3 \\
b & 1 & 4 & 4 \\
...
\textbf{\(i\)} & \textbf{\(L[i]\)} & \textbf{Rank in \(L\)} & \textbf{Calculation} & \textbf{\(LF[i]\)} \\
\hline
0 & a & 1 & cum[a]=1 + (1-1) = 1 & 1 \\
1 & n & 1 & cum[n]=5 + (1-1) = 5 & 5 \\
2 & n & 2 & cum[n]=5 + (2-1) = 6 & 6 \\
3 & b & 1 & cum[b]=4 + (1-1) = 4 & 4 \\
4 & \$ & 1 & cum[\$]=0 + (1-1) = 0 & 0 \\
5 & a & 2 & cum[a]=1 + (2-1) = 2 & 2 \\
6 & a & 3 & cum[a]=1 + (3-1) = 3 & 3 \\
\end{tabular}
\end{center}
