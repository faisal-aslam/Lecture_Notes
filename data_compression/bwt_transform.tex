...
\end{tabular}
\end{center}
\small\emph{Note: Rotation \#0 is the original string (no rotation). The last column will become the BWT output after sorting.}
\end{minipage}
...
2 & ana\$ban & 3 & n \\
3 & anana\$b & 1 & b \\
\rowcolor{originalrowcolor} 4 & banana\$ & 0 & \$ \\
5 & na\$bana & 4 & a \\
6 & nana\$ba & 2 & a \\
\hline
\end{tabular}
\end{center}
\small\emph{The row with \colorbox{originalrowcolor}{blue background} contains the \textbf{original string} (banana\$).}
\end{minipage}
...
\begin{tabular}{|c|>{\ttfamily}c|>{\columncolor{bwtoutputcolor}}c|}
\hline
\textbf{Rank} & \textbf{Sorted Rotation} & \textbf{Last Character} \\
\hline
0 & \$banana & \textbf{\textcolor{blue}{a}} \\
1 & a\$banan & \textbf{\textcolor{blue}{n}} \\
2 & ana\$ban & \textbf{\textcolor{blue}{n}} \\
3 & anana\$b & \textbf{\textcolor{blue}{b}} \\
4 & banana\$ & \textbf{\textcolor{blue}{\$}} \\
5 & na\$bana & \textbf{\textcolor{blue}{a}} \\
6 & nana\$ba & \textbf{\textcolor{blue}{a}} \\
\hline
\end{tabular}
\end{center}
...
\vspace{0.3cm}
\normalsize
\textbf{Important:} The primary index \(I = 4\) (0-based), which is the rank of the row containing the original string \texttt{banana\$}.
...
\begin{itemize}
    \item Each row is a cyclic rotation of the original string
    \item The first column \(F\) contains the first character of each rotation
    \item The last column \(L\) contains the last character of each rotation
    \item The LF-mapping tells us: from a character in \(L\), where does the \textbf{same occurrence} appear in \(F\)?
\end{itemize}

Here's the crucial observation:

\begin{importantbox}
For any row \(i\) in the sorted matrix, the character \(L[i]\) (last column) is the character that \textbf{immediately precedes} the character \(F[i]\) (first column) in the original string.

Why? Because row \(i\) is a rotation. If you take the last character and move it to the front, you get the rotation that starts with that character. The LF-mapping takes you to that row.
\end{importantbox}

This gives us a way to reconstruct the string backwards:

\begin{enumerate}
    \item Start at the row containing the original string (we know this row index = \(I\))
    \item The last character of that row, \(L[I]\), is the \textbf{last character} of the original string
    \item Use LF-mapping to find where this same character appears in \(F\) — that row's rotation starts with this character
    \item The last character of \textit{that} row is the character that comes \textbf{before} it in the original string
    \item Repeat — each step gives us the previous character
\end{enumerate}
...
\label{alg:bwt_inverse}
\begin{algorithmic}[1]
\Require \(L[0..n-1]\) (last column), \(I\) (index of original row)
\Ensure Original string \(S\)

\State \(n \gets \text{length}(L)\)
\State Compute LF-mapping array \(LF[0..n-1]\) using Algorithm~\ref{alg:lf-mapping}
\State Initialize \(S\) as empty string
\State \(row \gets I\)

\For{\(k \gets 0\) to \(n-1\)}
    \State Prepend \(L[row]\) to \(S\) \Comment{Add character to front (building backwards)}
    \State \(row \gets LF[row]\) \Comment{Move to the row that starts with this character}
\EndFor
...
\subsection*{Why We Prepend (Not Append)}

Notice that we \textbf{prepend} characters to \(S\) rather than appending. Here's why:

\begin{itemize}
    \item We start at the row containing the original string. Its last character \(L[I]\) is the \textbf{last} character of the original string.
    \item Each LF step takes us to a row whose rotation starts with the character we just saw. The last character of that new row is the character that comes \textbf{before} it in the original string.
    \item So we discover characters from \textbf{last to first}
    \item Prepending builds the string in correct order: first character discovered becomes last, last discovered becomes first
\end{itemize}
...
\begin{itemize}
    \item Row 4 (since \(I=4\)) contains the original string: \texttt{anana\$b}
    \item Its last character \(L[4] = \$\) is the last character of the original string
    \item The LF-mapping tells us: the same '\$' appears in \(F\) at row \(LF[4] = 0\)
    \item Row 0's rotation starts with '\$': \texttt{\$banana}
    \item Its last character \(L[0] = a\) is the character that comes \textbf{before} '\$' in the original string
    \item Repeat to discover characters backwards
\end{itemize}
...
\begin{itemize}
    ...
    \item Primary index I = 4 (tells us original string is at row 4)
\end{itemize}
...
At each step:
\begin{enumerate}
    \item Take \(L[row]\) — this is the next character we discovered (from end to beginning)
    \item Follow \(LF[row]\) to find the row whose rotation starts with this character
    \item That row's last character is the character that comes before it
\end{enumerate}

\begin{center}
\begin{tabularx}{\textwidth}{|c|c|c|c|X|}
\hline
\textbf{Step} & \textbf{Current Row} & \textbf{L[row]} & \textbf{Next Row = LF[row]} & \textbf{Explanation} \\
\hline
...
To get the original forward order, we \textbf{prepend} each newly discovered character:

\begin{center}
\begin{tabular}{|c|c|c|}
\hline
\textbf{Step} & \textbf{Character Discovered} & \textbf{String After Prepending} \\
\hline
1 & \$ & "\$" \\
2 & a & "a\$" \\
3 & n & "na\$" \\
...
\subsection*{Why This Works (Revisited)}
...
Walking this cycle backwards (by following LF) and collecting \(L[row]\) at each step reconstructs the original string from last character to first. Prepending each character gives us the correct forward order.
...
\subsubsection{Why MTF? Why Not RLE Directly on BWT?}

A common question is: \emph{If BWT already groups identical characters, why not apply RLE immediately?}

The key issue is that BWT creates \textbf{clusters}, not true \textbf{runs}. Real BWT outputs rarely contain long uninterrupted sequences of the same symbol. For example:

\[
\text{BWT("mississippi")} = \texttt{ipssm\$pissii}
\]

All the `i`'s are nearby, but not consecutive. Direct RLE would mostly see runs of length~1, giving almost no compression.

\begin{importantbox}[The Insight]
BWT brings similar symbols \emph{close} to each other, but not \emph{together}.  
MTF converts this proximity into actual redundancy: whenever a symbol reappears within a short distance, MTF outputs \texttt{0} because that symbol is already at the front of the list. This produces long runs of~0s, which RLE compresses extremely well.

\textbf{Additional benefit:} After MTF, the alphabet collapses to very small integers (0--5 dominate). Thus RLE not only gains long runs but also \emph{smaller run symbols}. Encoding runs of \texttt{0} is far cheaper than encoding runs of characters like `i` or `s`.
\end{importantbox}

In short: \textbf{BWT provides structure, but MTF converts that structure into compressible redundancy.} Without MTF, BWT output is not meaningfully RLE--friendly; with MTF, it becomes ideal for both RLE and entropy coding.
...
% ============================================================
% NEW SECTION: REVERSING MOVE-TO-FRONT (MTF DECODING)
% ============================================================

\subsubsection{Stage 2 (Reverse): Move-To-Front Decoding}
...
\begin{definitionbox}[Move-To-Front Decoding]
The MTF decoder maintains the same list of symbols as the encoder (initially identical). For each input index \(y\) (starting from 0):
\begin{enumerate}
    \item Output the symbol at position \(y\) in the current list
    \item Move that symbol to the front of the list
\end{enumerate}
...
\begin{algorithm}[H]
\caption{Move-To-Front Decoding}
\begin{algorithmic}[1]
\Require Input index sequence \(Y[0..n-1]\) (from MTF encoder)
\Ensure Recovered symbol sequence \(X[0..n-1]\)

\State Initialize list \(L \gets \text{sorted}(\Sigma)\) \Comment{Same initial order as encoder}
\For{\(i \gets 0\) to \(n-1\)}
    \State \(p \gets Y[i]\) \Comment{Index from the encoded stream}
    \State \(X[i] \gets L[p]\) \Comment{Symbol at that position}
    \State Remove \(L[p]\) from list and insert at front
\EndFor
\State \Return \(X\)
\end{algorithmic}
...
\begin{examplebox}[Decoding the MTF Output from Our Example]
...
\begin{center}
\begin{tabular}{c|c|c}
\textbf{Input Index} & \textbf{Current List} & \textbf{Output Symbol} \\
\hline
0 & \([a, b, n, \$]\) & a \\
2 & \([a, b, n, \$]\) & n \\
0 & \([n, a, b, \$]\) & n \\
...
Concatenating these gives the string \texttt{annb\$aa}, which is exactly the original BWT output we started with! This confirms that the MTF decoding correctly reverses the encoding.

\end{examplebox}

\subsubsection{Why MTF Decoding is Simple and Fast}

Notice the symmetry between encoding and decoding:
\begin{itemize}
    \item \textbf{Encoding:} Symbol \(\rightarrow\) find index \(\rightarrow\) output index \(\rightarrow\) move symbol to front
    \item \textbf{Decoding:} Index \(\rightarrow\) find symbol at that index \(\rightarrow\) output symbol \(\rightarrow\) move that symbol to front
\end{itemize}

Both operations run in \(O(1)\) time if we implement the list with a linked structure or an array and a separate ranking array. In practice, with an alphabet of 256 symbols, even a linear scan (\(O(|\Sigma|)\)) is negligible. This efficiency is one reason MTF is so popular in BWT-based compressors like \texttt{bzip2}.
