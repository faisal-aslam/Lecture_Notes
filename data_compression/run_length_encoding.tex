...
\begin{itemize}
    \item Position 1-5: \texttt{AAAAA} (5 A's) → run length 5 ≥ 3
    \begin{itemize}
    ...
    \item Position 10-13: \texttt{FFFF} (4 F's) → run length 4 ≥ 3
    \begin{itemize}
...
Input: \texttt{ABC\xFF\xFFDEF} (contains two 0xFF bytes)
...
\begin{itemize}
    ...
    \item \texttt{\xFF} (first) → this is the escape symbol! Need to escape it
    \begin{itemize}
        \item Output: \texttt{FF FF} (double escape means literal FF)
    \end{itemize}
    \item \texttt{\xFF} (second) → another escape symbol
    \begin{itemize}
