...
\end{itemize}
When the context is clear from context (e.g., in examples with specific strings), we use 
...
\While{$k \geq 0$}
    ...
    \If{$x \in S_c \setminus S_{\mathrm{excl}}$}
        \State $P(x \mid c, \mathit{excl}) \gets
               \dfrac{\text{Count}(c,x)}{N_c + D_c}$
               \Comment{PPMC: escape weight $= D_c$}
...
\begin{itemize}
    ...
    \item It \textbf{never assigns zero probability} to anything (escape always $>0$ 
          once $D_c > 0$, and unseen symbols get probability via escape chain)
...
\subsubsection{Example 1: A Simple Classroom Walkthrough — Encoding \texttt{"banana"}}

This example is designed to be presented in class without getting bogged down in 
mathematical details. We'll use PPM with maximum order $M=2$ and PPMC
for escape estimation. The alphabet size is $|\mathcal{A}| = 256$ (bytes), so
initial uniform probability for any symbol is $1/256 \approx 8$ bits.

\begin{examplebox}[PPM in Action: The Core Idea — Encoding "banana"]

\textbf{Step 1 — First symbol: \texttt{'b'}}
\begin{itemize}
    \item No history exists yet. We have no contexts to use.
    \item We must use a uniform distribution over all 256 symbols.
    \item Cost: $-\log_2(1/256) = 8$ bits.
\end{itemize}
After encoding, we update our models:
...
\begin{itemize}
    \item Current longest context: order-1 = \texttt{"b"} — empty $\Rightarrow$ escape
    \item Order-0: $D_{\emptyset}=1$, $N_{\emptyset}=1$; $P(\text{esc}) = 1/(1+1) = 1/2$ (1 bit)
    \item \texttt{'a'} is new to order-0 $\Rightarrow$ encode escape
    \item After escaping order-0, fall back to uniform: $1/256 \approx 8$ bits
    \item Total for 'a': $1 + 1 + 8 = 10$ bits
\end{itemize}
...
\begin{itemize}
    \item Current longest: order-2 \texttt{"ba"} — empty $\Rightarrow$ escape (cost 0 bits, since probability = 1)
    \item Next: order-1 \texttt{"a"} — empty $\Rightarrow$ escape (cost 0 bits)
    \item Order-0: $D_{\emptyset}=2$, $N_{\emptyset}=2$; $P(\text{esc}) = 2/(2+2) = 1/2$ (1 bit)
    \item \texttt{'n'} is new $\Rightarrow$ encode this escape
    \item Fallback to uniform: 8 bits
    \item Total: $0 + 0 + 1 + 8 = 9$ bits
\end{itemize}
...
\begin{itemize}
    \item Current longest: order-2 \texttt{"an"} — empty $\Rightarrow$ escape
    \item Next: order-1 \texttt{"n"} — empty $\Rightarrow$ escape
    \item Order-0: $D_{\emptyset}=3$, $N_{\emptyset}=3$; 
    \item \textbf{Critical observation:} \texttt{'a'} is IN order-0's table!
...
\begin{itemize}
    \item Current longest: order-2 \texttt{"na"} — empty $\Rightarrow$ escape
...
\hline
\texttt{b} (1) & uniform & 8.00 \\
\texttt{a} (2) & escapes + uniform & 10.00 \\
\texttt{n} (3) & escapes + uniform & 9.00 \\
\texttt{a} (4) & order-0 & 2.58 \\
\texttt{n} (5) & order-1 (\texttt{"a"}) & 1.00 \\
\texttt{a} (6) & order-2 (\texttt{"an"}) & 1.00 \\
\hline
\textbf{Total} & & \textbf{31.58 bits} \\
\end{tabular}
\end{center}

\textbf{What just happened?} The tables reveal the learning process:
\begin{itemize}
    \item \textbf{Steps 1-3}: Building contexts from scratch — expensive (8-10 bits)
    \item \textbf{Step 4}: Order-0 finally has enough data to predict 'a' (2.58 bits)
    \item \textbf{Step 5}: The pattern "a → n" is learned — order-1 predicts perfectly (1 bit)
    \item \textbf{Step 6}: The pattern "an → a" is learned — order-2 predicts perfectly (1 bit)
\end{itemize}
PPM discovered the alternating pattern "a","n","a","n" and exploited it immediately!
\end{examplebox}
...
\begin{itemize}
    \item No context available; use uniform distribution.
    \item Cost: $-\log_2(1/256) = 8$ bits.
\end{itemize}
After encoding, update models:
...
\begin{itemize}
    ...
    \item Order-0: $D_{\emptyset}=1$, $N_{\emptyset}=1$; $P(\text{esc}) = 1/(1+1) = 1/2$ (1 bit)
    \item \texttt{'b'} unseen in order-0 $\Rightarrow$ encode escape
    \item Fallback to uniform: 8 bits
    \item Total cost: $1 + 8 = 9$ bits
\end{itemize}
After encoding, update models:
...
\begin{itemize}
    ...
    \item Order-0: $D_{\emptyset}=2$, $N_{\emptyset}=2$; $P(\text{esc}) = 2/(2+2) = 1/2$ (1 bit)
    \item \texttt{'r'} unseen in order-0 $\Rightarrow$ encode escape
    \item Fallback to uniform: 8 bits
    \item Total cost: $1 + 8 = 9$ bits
\end{itemize}
After encoding, update models:
...
\begin{itemize}
    ...
    \item Next: order-1 \texttt{(a)} — has \texttt{b}:1 ($D_{\texttt{a}}=1$, $N_{\texttt{a}}=1$); $P(\text{esc}) = 1/2$ (1 bit)
    \item \texttt{'c'} unseen $\Rightarrow$ encode escape
    \item Order-0: $D_{\emptyset}=3$, $N_{\emptyset}=4$; $P(\text{esc}) = 3/(4+3) = 3/7 \approx 1.22$ bits
    \item \texttt{'c'} unseen $\Rightarrow$ encode escape
    \item Fallback to uniform: 8 bits
    \item Total cost: $1 + 1.22 + 8 = 10.22$ bits
\end{itemize}
After encoding, update models:
...
\begin{itemize}
    ...
    \item Order-0: $D_{\emptyset}=4$, $N_{\emptyset}=5$; $P(\texttt{a}) = 2/(5+4) = 2/9 \approx 2.17$ bits
...
\begin{itemize}
    ...
    \item Next: order-1 \texttt{(a)} — has \texttt{b}:1, \texttt{c}:1 ($D_{\texttt{a}}=2$, $N_{\texttt{a}}=2$); $P(\text{esc}) = 2/(2+2) = 1/2$ (1 bit)
    \item \texttt{'d'} unseen $\Rightarrow$ encode escape
    \item Order-0: $D_{\emptyset}=4$, $N_{\emptyset}=6$; $P(\text{esc}) = 4/(6+4) = 4/10 = 0.4$ (1.32 bits)
    \item \texttt{'d'} unseen $\Rightarrow$ encode escape
    \item Fallback to uniform: 8 bits
    \item Total cost: $1 + 1.32 + 8 = 10.32$ bits
\end{itemize}
After encoding, update models:
...
\begin{itemize}
    ...
    \item Order-0: $D_{\emptyset}=5$, $N_{\emptyset}=7$; $P(\texttt{a}) = 3/(7+5) = 3/12 = 0.25$ (2 bits)
...
\textbf{Symbol} & \textbf{Context used} & \textbf{Cost (bits)} \\
\hline
\texttt{a} (1) & uniform fallback & 8.00 \\
\texttt{b} (2) & escapes + uniform & 9.00 \\
\texttt{r} (3) & escapes + uniform & 9.00 \\
\texttt{a} (4) & order-0 & 2.58 \\
\texttt{c} (5) & escapes + uniform & 10.22 \\
\texttt{a} (6) & order-0 & 2.17 \\
\texttt{d} (7) & escapes + uniform & 10.32 \\
\texttt{a} (8) & order-0 & 2.00 \\
\texttt{b} (9) & order-1 (\texttt{"a"}) & 2.58 \\
\texttt{r} (10) & order-2 (\texttt{"ab"}) & 1.00 \\
\texttt{a} (11) & order-2 (\texttt{"br"}) & 1.00 \\
\hline
\textbf{Total} & & \textbf{57.87 bits} \\
\end{tabular}
\end{center}
...
\hline
1: \texttt{a} & uniform & 8.00 & (1,1) & (0,0) & (0,0) \\
2: \texttt{b} & escapes + uniform & 9.00 & (2,2) & (1,1) & (0,0) \\
3: \texttt{a} & order-0 & 2.00 & (2,3) & (1,1) & (1,1) \\
...
\begin{itemize}
    ...
    \item Total: $8+9+2+1+1+0.58+0.58+0.42+0.42+0.32 \approx 23.3$ bits ($2.33$ bpc)
\end{itemize}
