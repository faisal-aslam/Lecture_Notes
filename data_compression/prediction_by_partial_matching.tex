...
\begin{itemize}
    \item Order-0 (empty context): $D_{\emptyset} = 1$, $N_{\emptyset} = 1$, with \texttt{b}:1
    \item History: \texttt{"b"}
\end{itemize}

\textbf{Step 2 — Second symbol: \texttt{'a'}}
\begin{itemize}
    \item Current longest context: order-1 = \texttt{"b"}
    \item Context \texttt{"b"} is empty (never seen before) $\rightarrow$ send escape
    \item Order-0 now: $D_{\emptyset} = 1$, $N_{\emptyset} = 1$
    \item $P(\text{escape})$ at order-0 = $D_{\emptyset}/(N_{\emptyset} + D_{\emptyset}) = 1/(1+1) = 1/2$ $\rightarrow$ 1 bit
    \item \texttt{'a'} is new to order-0 $\rightarrow$ encode this escape
    \item After escaping order-0, we fall back to uniform: $1/256 \approx 8$ bits
\end{itemize}
Total for 'a': $1$ (escape at order-1) $+ 1$ (escape at order-0) $+ 8$ (uniform) $= 10$ bits.

After encoding, update:
\begin{itemize}
    \item Order-1 context \texttt{"b"}: now has \texttt{a} with count 1 ($D_{\texttt{b}} = 1$, $N_{\texttt{b}} = 1$)
    \item Order-0: \texttt{a} count = 1 (so now $D_{\emptyset} = 2$, $N_{\emptyset} = 2$)
    \item History: \texttt{"ba"}
\end{itemize}

\textbf{Step 3 — Third symbol: \texttt{'n'}}
\begin{itemize}
    \item Current longest context: order-2 = \texttt{"ba"}
    \item Context \texttt{"ba"} is empty $\rightarrow$ escape
    \item Next context: order-1 = \texttt{"a"} (last symbol)
    \item Context \texttt{"a"} is empty $\rightarrow$ escape
    \item Order-0: now has \{\texttt{b:1, a:1}\}, $D_{\emptyset} = 2$, $N_{\emptyset} = 2$
    \item $P(\text{escape})$ at order-0 = $2/(2+2) = 2/4 = 1/2$ (1 bit)
    \item \texttt{'n'} is new $\rightarrow$ encode this escape
    \item Fallback to uniform: 8 bits
\end{itemize}
Total = 1 (escape at order-0) + 8 (uniform) = 9 bits.

After encoding, update:
\begin{itemize}
    \item Order-2 \texttt{"ba"}: now has \texttt{n} with count 1 ($D_{\texttt{ba}} = 1$, $N_{\texttt{ba}} = 1$)
    \item Order-1 \texttt{"a"}: now has \texttt{n} with count 1 ($D_{\texttt{a}} = 1$, $N_{\texttt{a}} = 1$)
    \item Order-0: \texttt{n} count = 1 ($D_{\emptyset} = 3$, $N_{\emptyset} = 3$)
    \item History: \texttt{"ban"}
\end{itemize}

\textbf{Step 4 — Fourth symbol: \texttt{'a'}}
\begin{itemize}
    \item Current longest context: order-2 = \texttt{"an"}
    \item Context \texttt{"an"} is empty $\rightarrow$ escape
    \item Next context: order-1 = \texttt{"n"}
    \item Context \texttt{"n"} is empty $\rightarrow$ escape
    \item Order-0: now has \{\texttt{b:1, a:1, n:1}\}, $D_{\emptyset} = 3$, $N_{\emptyset} = 3$
    \item $P(\text{escape}) = 3/(3+3) = 3/6 = 1/2$ (1 bit) — but wait!
    \item \textbf{Critical observation:} \texttt{'a'} is actually IN order-0's table!
    \item So we don't escape — we encode \texttt{'a'} directly using order-0
    \item $P(\texttt{a})$ at order-0 = $\text{Count}(\emptyset,\texttt{a})/(N_{\emptyset} + D_{\emptyset}) = 1/(3+3) = 1/6 \approx 2.58$ bits
\end{itemize}
Total = 2.58 bits (no escapes needed — we found 'a' at order-0).

After encoding, update:
\begin{itemize}
    \item Order-2 \texttt{"an"}: now has \texttt{a} with count 1 ($D_{\texttt{an}} = 1$, $N_{\texttt{an}} = 1$)
    \item Order-1 \texttt{"n"}: now has \texttt{a} with count 1 ($D_{\texttt{n}} = 1$, $N_{\texttt{n}} = 1$)
    \item Order-0: \texttt{a} count increases to 2 ($N_{\emptyset} = 4$)
    \item History: \texttt{"bana"}
\end{itemize}

\textbf{Step 5 — Fifth symbol: \texttt{'n'}}
\begin{itemize}
    \item Current longest context: order-2 = \texttt{"na"}
    \item Context \texttt{"na"} is empty $\rightarrow$ escape
    \item Next context: order-1 = \texttt{"a"}
    \item \textbf{Key moment:} Context \texttt{"a"} now exists! It has seen \texttt{n} once before (from step 3)
    \item At order-1 \texttt{"a"}: $D_{\texttt{a}} = 1$ (only \texttt{n}), $N_{\texttt{a}} = 1$ (one occurrence)
    \item $P(\texttt{n})$ at order-1 = $\text{Count}(\texttt{"a"},\texttt{n})/(N_{\texttt{a}} + D_{\texttt{a}}) = 1/(1+1) = 1/2 = 1$ bit
    \item We found our symbol! No escapes needed.
\end{itemize}
Total = 1 bit!

After encoding, update:
\begin{itemize}
    \item Order-2 \texttt{"na"}: now has \texttt{n} with count 1 ($D_{\texttt{na}} = 1$, $N_{\texttt{na}} = 1$)
    \item Order-1 \texttt{"a"}: \texttt{n} count increases to 2 ($N_{\texttt{a}} = 2$)
    \item Order-0: \texttt{n} count increases to 2 ($N_{\emptyset} = 5$)
    \item History: \texttt{"banan"}
\end{itemize}

\textbf{Step 6 — Sixth symbol: \texttt{'a'}}
\begin{itemize}
    \item Current longest context: order-2 = \texttt{"an"}
    \item Context \texttt{"an"} exists! It has seen \texttt{a} once before (from step 4)
    \item At order-2 \texttt{"an"}: $D_{\texttt{an}} = 1$ (only \texttt{a}), $N_{\texttt{an}} = 1$ (one occurrence)
    \item $P(\texttt{a})$ at order-2 = $1/(1+1) = 1/2 = 1$ bit
    \item Found at highest order — perfect prediction!
\end{itemize}
Total = 1 bit!
...
\hline
\texttt{b} & uniform & 8.00 \\
\texttt{a} & escapes + uniform & 10.00 \\
\texttt{n} & escapes + uniform & 9.00 \\
\texttt{a} & order-0 & 2.58 \\
\texttt{n} & order-1 (\texttt{"a"}) & 1.00 \\
\texttt{a} & order-2 (\texttt{"an"}) & 1.00 \\
\hline
Total & & 31.58 bits \\
\end{tabular}
\end{center}
...
The first three symbols cost many bits because we were building our context models. 
By symbol 4, we had enough history to start making useful predictions. By symbols 5 and 6, 
PPM had learned the pattern "after 'a' comes 'n', after 'n' comes 'a'" and could predict 
with near-perfect accuracy (1 bit per symbol). This is the power of PPM — it learns 
patterns dynamically and exploits them as soon as they appear.
\end{examplebox}
...
\boxheading{Initialisation.} All models empty. History $H = \varepsilon$.

\bigskip
\noindent\textbf{Symbol 1: \texttt{'a'}}\\
No context available; use uniform. $P(\texttt{a}) = 1/256 \approx 8$ bits.\\
Update: Order-0: $\texttt{a}:1$, $D_{\emptyset}=1$, $N_{\emptyset}=1$. $H =$ \texttt{a}.
...
\begin{itemize}
    ...
    \item Order-0: $D_{\emptyset}=1$, $N_{\emptyset}=1$; $P(\text{esc})=1/2$ (1 bit).
          \texttt{b} unseen $\Rightarrow$ escape.
    \item Fallback: $P(\texttt{b}) \approx 1/256 \approx 8$ bits.
\end{itemize}
\textbf{Total} $\approx 1 + 8 = 9$ bits.\\
Update: O-1(\texttt{a}): \texttt{b}:1 ($D_{\texttt{a}}=1$, $N_{\texttt{a}}=1$); 
O-0: \texttt{b}:1 ($D_{\emptyset}=2$, $N_{\emptyset}=2$). $H =$ \texttt{ab}.
...
\begin{itemize}
    ...
    \item Order-0: $D_{\emptyset}=2$, $N_{\emptyset}=2$; $P(\text{esc})=2/4=0.5$ (1 bit).
          \texttt{r} unseen $\Rightarrow$ escape.
    \item Fallback: $\approx 8$ bits.
\end{itemize}
\textbf{Total} $\approx 1 + 8 = 9$ bits.\\
Update: O-2(\texttt{ab}):\texttt{r}:1 ($D_{\texttt{ab}}=1$, $N_{\texttt{ab}}=1$); 
O-1(\texttt{b}):\texttt{r}:1 ($D_{\texttt{b}}=1$, $N_{\texttt{b}}=1$);
O-0: \texttt{r}:1 ($D_{\emptyset}=3$, $N_{\emptyset}=3$). $H =$ \texttt{abr}.
...
\begin{itemize}
    ...
    \item Order-0: \texttt{a}:1, $D_{\emptyset}=3$, $N_{\emptyset}=3$; denom $= 3+3=6$.
          $P(\texttt{a}) = 1/6 \approx 2.58$ bits. \texttt{a} found.
\end{itemize}
\textbf{Total} $\approx 2.58$ bits.\\
Update: O-2(\texttt{br}):\texttt{a}:1 ($D_{\texttt{br}}=1$, $N_{\texttt{br}}=1$); 
O-1(\texttt{r}):\texttt{a}:1 ($D_{\texttt{r}}=1$, $N_{\texttt{r}}=1$);
O-0: \texttt{a}:2 ($N_{\emptyset}=4$). $H =$ \texttt{abra}.
...
\begin{itemize}
    ...
    \item Order-1 \texttt{(a)}: \texttt{b}:1 ($D_{\texttt{a}}=1$, $N_{\texttt{a}}=1$); denom $=2$.
          $P(\text{esc})=1/2 \approx 1$ bit. \texttt{c} unseen.
    \item Order-0: $D_{\emptyset}=3$, $N_{\emptyset}=4$; denom $=7$.
          $P(\text{esc})=3/7 \approx 1.22$ bits. \texttt{c} unseen.
    \item Fallback: $\approx 8$ bits.
\end{itemize}
\textbf{Total} $\approx 1 + 1.22 + 8 = 10.22$ bits.\\
Update: \texttt{c} added to all models; 
O-2(\texttt{ra}):\texttt{c}:1 ($D_{\texttt{ra}}=1$, $N_{\texttt{ra}}=1$);
O-1(\texttt{a}):\texttt{c}:1 ($D_{\texttt{a}}=2$, $N_{\texttt{a}}=2$);
O-0: \texttt{c}:1 ($D_{\emptyset}=4$, $N_{\emptyset}=5$). $H =$ \texttt{abrac}.
...
\begin{itemize}
    ...
    \item Order-0: \texttt{a}:2, $D_{\emptyset}=4$, $N_{\emptyset}=5$; denom $=9$.
          $P(\texttt{a})=2/9 \approx 2.17$ bits.
\end{itemize}
\textbf{Total} $\approx 2.17$ bits.\\
Update: O-0: \texttt{a}:3 ($N_{\emptyset}=6$). $H =$ \texttt{abraca}.
...
\begin{itemize}
    ...
    \item Order-1 \texttt{(a)}: \texttt{b}:1, \texttt{c}:1 ($D_{\texttt{a}}=2$, $N_{\texttt{a}}=2$);
          denom $=4$. $P(\text{esc})=2/4=0.5 \approx 1$ bit. \texttt{d} unseen.
    \item Order-0: $D_{\emptyset}=4$, $N_{\emptyset}=6$; denom $=10$.
          $P(\text{esc})=4/10=0.4 \approx 1.32$ bits. \texttt{d} unseen.
    \item Fallback: $\approx 8$ bits.
\end{itemize}
\textbf{Total} $\approx 1 + 1.32 + 8 = 10.32$ bits.\\
Update: \texttt{d} added; 
O-2(\texttt{ca}):\texttt{d}:1 ($D_{\texttt{ca}}=1$, $N_{\texttt{ca}}=1$);
O-1(\texttt{a}):\texttt{d}:1 ($D_{\texttt{a}}=3$, $N_{\texttt{a}}=3$);
O-0: \texttt{d}:1 ($D_{\emptyset}=5$, $N_{\emptyset}=7$). $H =$ \texttt{abracad}.
...
\begin{itemize}
    ...
    \item Order-0: \texttt{a}:3, $D_{\emptyset}=5$, $N_{\emptyset}=7$; denom $=12$.
          $P(\texttt{a})=3/12=0.25 \approx 2$ bits.
\end{itemize}
\textbf{Total} $\approx 2$ bits.\\
Update: O-0: \texttt{a}:4 ($N_{\emptyset}=8$). $H =$ \texttt{abracada}.
...
\begin{itemize}
    ...
    \item Order-1 \texttt{(a)}: \texttt{b}:1, \texttt{c}:1, \texttt{d}:1
          ($D_{\texttt{a}}=3$, $N_{\texttt{a}}=3$); denom $=6$.
          $P(\texttt{b})=1/6 \approx 2.58$ bits. \texttt{b} found.
\end{itemize}
\textbf{Total} $\approx 2.58$ bits.\\
Update: O-1(\texttt{a}): \texttt{b}:2 ($N_{\texttt{a}}=4$); O-0: \texttt{b}:2 ($N_{\emptyset}=9$).
$H =$ \texttt{abracadab}.
...
$H =$ \texttt{abracadab}.
\begin{itemize}
    \item Order-2 \texttt{(ab)}: \texttt{r}:1 ($D_{\texttt{ab}}=1$, $N_{\texttt{ab}}=1$); denom $=2$.
          $P(\texttt{r})=1/2 = 1$ bit. \texttt{r} found.
\end{itemize}
\textbf{Total} $= 1$ bit.\\
Update: O-2(\texttt{ab}): \texttt{r}:2 ($N_{\texttt{ab}}=2$); O-0: \texttt{r}:2 ($N_{\emptyset}=10$).
$H =$ \texttt{abracadabr}.
...
$H =$ \texttt{abracadabr}.
\begin{itemize}
    \item Order-2 \texttt{(br)}: \texttt{a}:1 ($D_{\texttt{br}}=1$, $N_{\texttt{br}}=1$); denom $=2$.
          $P(\texttt{a})=1/2 = 1$ bit.
\end{itemize}
\textbf{Total} $= 1$ bit.\\
Update: O-2(\texttt{br}): \texttt{a}:2 ($N_{\texttt{br}}=2$); O-0: \texttt{a}:5 ($N_{\emptyset}=11$).

\bigskip
\noindent\textbf{Summary (Example~2).}
\begin{verbatim}
Symbol | Context used          | Code bits (approx)
-------+-----------------------+--------------------
  a    | uniform fallback      |  8.00
  b    | escapes + uniform     |  9.00
  r    | escapes + uniform     |  9.00
  a    | order-0               |  2.58
  c    | escapes + uniform     | 10.22
  a    | order-0               |  2.17
  d    | escapes + uniform     | 10.32
  a    | order-0               |  2.00
  b    | order-1 ("a")         |  2.58
  r    | order-2 ("ab")        |  1.00
  a    | order-2 ("br")        |  1.00
...
$D_{\emptyset}=1$, $N_{\emptyset}=1$ (\texttt{a}:1). $H=$ \texttt{a}.

\noindent\textbf{Symbol 2 -- \texttt{'b'}:} O-1(\texttt{a}) empty, esc; O-0
$P(\text{esc}) = D_{\emptyset}/(N_{\emptyset}+D_{\emptyset}) = 1/2$ (1~bit), 
\texttt{b} unseen $\Rightarrow$ fallback 8~bits. Total $\approx 9$ bits.
Update: O-1(\texttt{a}):\texttt{b}:1 ($D_{\texttt{a}}=1$, $N_{\texttt{a}}=1$); 
O-0:\texttt{b}:1 ($D_{\emptyset}=2$, $N_{\emptyset}=2$).

\noindent\textbf{Symbol 3 -- \texttt{'a'}:} O-1(\texttt{b}) empty, esc;
O-0: \texttt{a}:1, $D_{\emptyset}=2$, $N_{\emptyset}=2$, denom $4$, $P(\texttt{a})=1/4 = 2$ bits.
Update: O-1(\texttt{b}):\texttt{a}:1 ($D_{\texttt{b}}=1$, $N_{\texttt{b}}=1$);
O-0: \texttt{a}:2 ($N_{\emptyset}=3$).

\noindent\textbf{Symbol 4 -- \texttt{'b'}:} O-1(\texttt{a}):\texttt{b}:1
($D_{\texttt{a}}=1$, $N_{\texttt{a}}=1$), $P(\texttt{b})=1/2 = 1$ bit.
Update: O-1(\texttt{a}): \texttt{b}:2 ($N_{\texttt{a}}=2$); O-0: \texttt{b}:2 ($N_{\emptyset}=4$).

\noindent\textbf{Symbol 5 -- \texttt{'a'}:} O-1(\texttt{b}):\texttt{a}:1
($D_{\texttt{b}}=1$, $N_{\texttt{b}}=1$), $P(\texttt{a})=1/2 = 1$ bit.
Update: O-1(\texttt{b}): \texttt{a}:2 ($N_{\texttt{b}}=2$); O-0: \texttt{a}:3 ($N_{\emptyset}=5$).

\noindent\textbf{Symbol 6 -- \texttt{'b'}:} O-1(\texttt{a}):\texttt{b}:2
($D_{\texttt{a}}=1$, $N_{\texttt{a}}=2$), $P(\texttt{b})=2/3 \approx 0.58$ bits.
Update: O-1(\texttt{a}): \texttt{b}:3 ($N_{\texttt{a}}=3$); O-0: \texttt{b}:3 ($N_{\emptyset}=6$).

\noindent\textbf{Symbol 7 -- \texttt{'a'}:} O-1(\texttt{b}):\texttt{a}:2
($D_{\texttt{b}}=1$, $N_{\texttt{b}}=2$), $P(\texttt{a})=2/3 \approx 0.58$ bits.
Update: O-1(\texttt{b}): \texttt{a}:3 ($N_{\texttt{b}}=3$); O-0: \texttt{a}:4 ($N_{\emptyset}=7$).

\noindent\textbf{Symbol 8 -- \texttt{'b'}:} O-1(\texttt{a}):\texttt{b}:3
($D_{\texttt{a}}=1$, $N_{\texttt{a}}=3$), $P(\texttt{b})=3/4 \approx 0.42$ bits.
Update: O-1(\texttt{a}): \texttt{b}:4 ($N_{\texttt{a}}=4$); O-0: \texttt{b}:4 ($N_{\emptyset}=8$).

\noindent\textbf{Symbol 9 -- \texttt{'a'}:} O-1(\texttt{b}):\texttt{a}:3
($D_{\texttt{b}}=1$, $N_{\texttt{b}}=3$), $P(\texttt{a})=3/4 \approx 0.42$ bits.
Update: O-1(\texttt{b}): \texttt{a}:4 ($N_{\texttt{b}}=4$); O-0: \texttt{a}:5 ($N_{\emptyset}=9$).

\noindent\textbf{Symbol 10 -- \texttt{'b'}:} O-1(\texttt{a}):\texttt{b}:4
($D_{\texttt{a}}=1$, $N_{\texttt{a}}=4$), $P(\texttt{b})=4/5 \approx 0.32$ bits.
