...
The alphabet size is $|\mathcal{A}| = 256$ (bytes). We apply the exclusion principle throughout.

\begin{examplebox}[PPM in Action: Encoding "banana" with Exclusion]

\textbf{Step 1 — First symbol: \texttt{'b'}}
\begin{itemize}
    \item No history exists yet. All contexts are empty.
    \item After escaping through all empty contexts (cost 0 bits each), we reach Order -1.
...
\begin{itemize}
    ...
    \item Order-0: $D_{\emptyset}=3$, $N_{\emptyset}=3$; \texttt{'a'} IS in order-0 table!
...
\begin{itemize}
    ...
    \item Savings from exclusion: $\approx 1.02$ bits
\end{itemize}
\end{examplebox}
