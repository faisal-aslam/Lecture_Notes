...
Shannon Fano coding. All three build a prefix code, but they go about
assigning codeword lengths differently. Shannon coding takes each symbol's
probability and rounds it up to the nearest integer number of bits using
$-\log_2 p$, then assigns codewords in order using a running sum of the
probabilities that came before it. Shannon-Fano coding instead works
top-down: it sorts symbols by probability, then repeatedly splits the list
into two halves with the most balanced total probability it can manage,
handing out a \texttt{0} or \texttt{1} for each split. Both are simple,
sensible heuristics, and both were stepping stones on the way to Huffman
coding, but neither one is guaranteed to produce the shortest possible
prefix code, the way Huffman coding is.

Huffman coding, by contrast, is fast and simple to build. It is also
...
exactly, with two ranked tables. While we're at it, let's bring back two
prefix codes we named earlier and then left on the sidelines: Shannon coding
and Shannon-Fano coding.

\engage{prefixcol}{Pause and guess:}{Shannon coding and Shannon-Fano coding
are both prefix codes, just like Huffman. Where do you think they'll land
on the ratio table below -- above Huffman, below it, or tied with it?}

\begin{figure}[H]
\centering
\begin{tabular}{@{}cp{3.0cm}p{8.0cm}@{}}
\toprule
...
So Shannon and Shannon-Fano land at the very bottom on ratio, behind even
Huffman. Now let's rank the same six algorithms a completely different way:
by speed.
...
Facebook and Netflix use it for everyday compression at scale.
