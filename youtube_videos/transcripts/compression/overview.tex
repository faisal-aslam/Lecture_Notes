...
prediction filter (\ref{sec:lossy-fill}). The discrete cosine transform
and the discrete wavelet transform, the workhorses behind JPEG, MP3, and
JPEG2000, do the same job for lossy compression: they convert a signal
into frequency components so that most of the energy lands on a few
large coefficients and everything else can be quantized away almost for
free. DCT does this with global, whole-block waves, while wavelets use
short, localized ripples that adapt to both frequency and position at
once, which is what lets them keep sharp edges crisp instead of
smearing them across the block. What unites all of these, BWT, MTF,
delta encoding, DCT, wavelets, is the same handoff pattern: rearrange or
re-express the data first, then let a simpler coder do the squeezing.
...
\caption{The transform-based branch. None of BWT, MTF, delta encoding,
DCT, or the wavelet transform compete with entropy coding or LZ -- each
is a pre-processing step that makes whatever comes after it work
better.}
