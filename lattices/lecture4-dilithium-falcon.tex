...
\end{align*}
Compare to the original $w = (16,3,5,2)$: they are \emph{not} identical --- $w' = w - c\cdot s_2 = w - (1,-1,0,0)$, matching the algebra of Section~4's remark exactly (subtract $s_2=1-x=(1,-1,0,0)$ from $w=(16,3,5,2)$: $(16-1,3-(-1),5,2)=(15,4,5,2)$ $=w'$, confirmed). In this toy example we accept the signature by checking this exact algebraic identity holds (the real scheme's hint mechanism is what lets a verifier who does \emph{not} know $s_2$ still confirm this identity, via the high-order bits of $w$ instead of $w$ itself --- omitted here for tractability, as flagged in Section~4).
\end{exbox}
