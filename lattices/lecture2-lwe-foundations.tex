...
\subsection{What the Error Distribution $\chi$ Actually Is}
...
Our worked examples below use a crude stand-in ($e\in\{-1,0,1\}$, each roughly equally likely) purely because it's easy to compute by hand. Real schemes use something bell-curve-shaped (a true discrete Gaussian, or in Kyber's case a closely related and cheaper-to-sample \emph{centered binomial distribution}) for a precise mathematical reason: the hardness proofs connecting LWE back to worst-case lattice problems (Regev's reduction, Section~4.4) require the noise to behave like a Gaussian. Swap in a different-shaped distribution and those proofs may simply no longer apply --- ``small errors'' alone isn't enough; their \emph{shape} matters too. This is a recurring theme you'll meet again in Session 5: real implementations must sample from exactly the right distribution, and subtle sampling mistakes (or side-channel leakage \emph{during} sampling) can quietly undermine security guarantees that look airtight on paper.
\end{rmkbox}
