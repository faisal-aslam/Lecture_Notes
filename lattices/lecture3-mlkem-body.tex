...
\begin{tikzpicture}[scale=0.78, every node/.style={font=\small}]
  ...
  \draw[-{Latex[length=1.8mm]}, thick] (n0) to[bend left=30] node[midway, sloped, above, font=\tiny] {$+\varepsilon=+3$} (n3);
\end{tikzpicture}
\\[2pt]
...
\begin{tikzpicture}[scale=0.78, every node/.style={font=\small}]
  ...
  \draw[dashed, gray] (0,0) -- (\boundang:2.65);
  \node[gray, font=\tiny] at (\boundang:2.95) {\small $q/4$};
  \draw[-{Latex[length=1.8mm]}, thick] (n0) to[bend left=30] node[midway, sloped, above, font=\tiny] {$+\varepsilon=+5$} (n5);
\end{tikzpicture}
\\[2pt]
