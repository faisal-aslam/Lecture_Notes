...
\begin{rmkbox}{Why $k\times k$, Not $k\times n$}
It's tempting to expect a $k\times n$ shape here, since the secret is ``$k$ polynomials, each of degree $<n$.'' But $n$ never appears as a matrix \emph{dimension} in the module setting --- it's absorbed \emph{inside} each entry. Every entry $A_{ij}$ is not a scalar; it is itself a full ring element of $R_q$, i.e.\ an $n$-coefficient polynomial. The matrix's outer shape, $k\times k$, only tracks how many ring-element \emph{equations} and ring-element \emph{unknowns} there are --- exactly the same shape logic as ordinary linear algebra, just with scalars replaced by ring elements throughout. Concretely: the total count of $\mathbb{Z}_q$ numbers stored in $A$ is $k\times k\times n$ (not $k\times n$) --- $k^2$ ring elements, each carrying $n$ coefficients. You already saw the smallest possible case of this in Lecture 2 and again below: with $k=1$, $A$ is a single ring element, e.g.\ $A=3+5x+2x^2+x^3$ --- one ``$1\times1$ matrix of ring elements,'' but that one entry already has $n=4$ coefficients packed inside it. (If you ``unfolded'' $A$ all the way down to plain scalars, each ring-element entry would expand into its own $n\times n$ block --- a structured, easy-to-multiply matrix --- so the true scalar picture is $kn\times kn$-ish underneath. Module-LWE's entire efficiency trick is hiding that internal $n\times n$ structure inside one opaque ring multiplication, so the matrix you actually write down only needs to track $k$.)
\end{rmkbox}
...
This seems backwards --- Lecture 2 spent an entire lecture explaining that noise is what makes LWE hard, and now we're adding \emph{more} noise on purpose. First, a scope check: compression here is applied to the \textbf{ciphertext} --- the $\mathrm{Compress}_{d_u}(\vec u)$, $\mathrm{Compress}_{d_v}(v)$ steps at the end of Enc, Section~\ref{sec:L3:the-cpa-secure-core-kyber-cpapke} below --- \emph{not} to the public key. The public-key sizes in the parameter table above come entirely from $k$ and $n$ (how many ring elements get stored), not from any compression step; $(A,\vec t)$ is never run through Compress/Decompress anywhere in this scheme. Compression matters most for the ciphertext specifically because a fresh ciphertext is produced on \emph{every single encryption}, while a public key is typically generated and transmitted once per session.

The resolution to the ``more noise on purpose'' worry: encryption already tolerates a noise budget (Section~\ref{sec:L3:why-decryption-works-the-noise-cancellat} below shows exactly how much, and exactly where that $q/4$ threshold comes from). Compression consumes a small, carefully bounded slice of that budget in exchange for a large reduction in ciphertext size. Being safe to do this is \emph{not} simply a matter of ``the failure probability happens to be low'' --- a low-but-unaccounted-for probability would not, by itself, be trustworthy. The real safety argument has two parts: (1) decryption fails only if the \emph{total} noise $\varepsilon$ (original LWE-style noise, from Section~\ref{sec:L3:why-decryption-works-the-noise-cancellat}) exceeds $q/4$ in absolute value --- a hard threshold fixed by the algebra, not a design choice; and (2) $d_u,d_v$ and the noise parameters $\eta_1,\eta_2$ are chosen at design time specifically so that the \emph{combined} error (original noise plus compression's rounding error) clears that $q/4$ bar with overwhelming margin. The astronomically small failure probability ML-KEM actually achieves (below $2^{-100}$) is the \emph{consequence} of that accounting, not an independent justification standing on its own. Push $d$ too low, though, and the accounted-for margin shrinks and decryption starts failing for real --- this is a real, quantifiable trade-off the standard's parameters were chosen to balance, and it's also precisely the kind of margin that a chosen-ciphertext attack (Lesson 5) tries to push past its limit on purpose.
\end{rmkbox}
...
\begin{rmkbox}{Where the $q/4$ Threshold Actually Comes From}
It's worth deriving this bound rather than just asserting it. $\mathrm{Encode}$ places every message bit at one of two ``poles'' on the circle $\mathbb{Z}_q$, as far apart as possible: bit $0\mapsto 0$, bit $1\mapsto \lceil q/2\rceil$. $\mathrm{Decode}$ then simply asks, for each noisy coefficient $m_i' = \mathrm{Encode}(m)_i + \varepsilon_i$: \emph{which pole is this closer to?} Picture $\mathbb{Z}_q$ as a clock face with $0$ at 12 o'clock and $q/2$ at 6 o'clock --- the two points exactly \emph{midway} between the poles sit at $q/4$ (3 o'clock) and $3q/4$ (9 o'clock). Those midpoints are precisely where ``closer to $0$'' flips over into ``closer to $q/2$,'' and vice versa.

So: if the true encoded value was $0$, $\mathrm{Decode}$ still returns the correct bit as long as the noisy coefficient hasn't crossed either nearby midpoint --- i.e.\ as long as $|\varepsilon_i| < q/4$. By the same symmetric argument around the other pole, a true value of $\lceil q/2\rceil$ still decodes correctly under the same condition, $|\varepsilon_i| < q/4$. In short, $q/4$ is not an arbitrary safety margin --- it is exactly \emph{half the gap between the two encoding poles} ($q/2$ apart, so $q/4$ of margin on each side before crossing into the other bit's territory). Push any coefficient of $\varepsilon$ past $q/4$ in absolute value, and that coefficient crosses a decision boundary, and $\mathrm{Decode}$ silently returns the wrong bit.
\end{rmkbox}
...
\begin{rmkbox}{Chosen-Ciphertext Access Is Broader Than ``Man-in-the-Middle''}
It's tempting to read ``CCA'' as ``secure against a man-in-the-middle,'' but that undersells the threat model, and it's worth being precise here before the defbox above becomes the FO transform below. What CCA security actually defends against is any setting where an attacker can submit ciphertexts of their own choosing --- not necessarily honestly produced by $\mathrm{Enc}$ --- to a decryption oracle and observe what comes back, whether that's the decrypted plaintext, an error, or just how long the operation took. A literal on-path man-in-the-middle is only \emph{one} way to get that access. Others include: a malicious server that a victim's client submits KEM ciphertexts against; one endpoint in a protocol who can replay or tamper with messages before they reach decryption; or, as Lesson 5 covers in detail, an attacker with physical access to a device and its side channels, with no network position at all. CPA security only ever assumes the weaker, more idealized attacker who watches outputs of $\mathrm{Enc}$ and never gets a decryption oracle in any of these forms --- a threat model too optimistic for essentially any real deployment, not merely one that happens to be vulnerable to MITM specifically. This is exactly why TLS, SSH, and every other real protocol require CCA security as a baseline, and why ML-KEM applies the FO transform below \emph{unconditionally} rather than as an optional hardening step reserved for ``risky'' settings.
\end{rmkbox}
