...
\label{sec:L2:symmetric-vs-asymmetric-cryptography}
...
\begin{defbox}{Symmetric-Key Cryptography}
Both parties share the \emph{same} secret key $k$, used both to encrypt and to decrypt (or to generate and to verify a message authentication code). Algorithms like AES and ChaCha20 are symmetric. Symmetric cryptography is fast --- often gigabytes per second on ordinary hardware --- but it has a bootstrapping problem built in: both parties need $k$ \emph{before} they can communicate securely, and getting $k$ from one party to the other, over a channel an attacker might be watching, is exactly the problem symmetric cryptography alone cannot solve.
\end{defbox}

\begin{defbox}{Asymmetric (Public-Key) Cryptography}
Each party generates a \emph{pair} of mathematically linked keys: a \textbf{public key}, safe to hand to anyone, including an attacker, and a \textbf{private key}, kept secret forever. Anyone with Bob's public key can encrypt a message only Bob's private key can decrypt --- or verify a signature only Bob's private key could have produced. No shared secret is ever needed in advance; the public key can be shouted across a room an eavesdropper is listening to, and security holds anyway. This is exactly the one-way-function machinery Section~\ref{sec:L2:one-way-functions-the-idea-behind-all-of} builds next: the public key is the ``easy forward direction,'' and only the matching private key makes the ``hard backward direction'' easy.
\end{defbox}
...
\begin{tikzpicture}[scale=0.82, every node/.style={font=\small}]
  \node[draw, minimum width=2.2cm, minimum height=1cm, fill=blue!8] (a) at (0,0) {Alice};
  \node[draw, minimum width=2.2cm, minimum height=1cm, fill=green!8] (b) at (5,0) {Bob};
  \node[draw, minimum width=1.6cm, minimum height=0.9cm, fill=orange!12] (k) at (2.5,-1.6) {key $k$};
  \draw[-{Latex[length=2mm]}, thick] (a) -- (b) node[midway, above, font=\tiny] {Encrypt$(m,k)$};
  \draw[-{Latex[length=2mm]}, thick, dashed] (k) -- (a);
  \draw[-{Latex[length=2mm]}, thick, dashed] (k) -- (b);
  \node[font=\tiny, align=center] at (2.5,-2.35) {same secret $k$,\\ known to both, no one else};
\end{tikzpicture}
\\[2pt]
{\small \textbf{Symmetric:} one shared key does both jobs.}
\end{minipage}%
\hfill
\begin{minipage}[t]{0.47\textwidth}
\centering
\begin{tikzpicture}[scale=0.82, every node/.style={font=\small}]
  \node[draw, minimum width=2.2cm, minimum height=1cm, fill=blue!8] (a) at (0,0) {Alice};
  \node[draw, minimum width=2.2cm, minimum height=1cm, fill=green!8] (b) at (5,0) {Bob};
  \node[draw, minimum width=1.9cm, minimum height=0.8cm, fill=green!15] (pk) at (2.5,-1.5) {$pk_B$ (public)};
  \node[draw, minimum width=1.9cm, minimum height=0.8cm, fill=red!10] (sk) at (5,-2.6) {$sk_B$ (private)};
  \draw[-{Latex[length=2mm]}, thick] (a) -- (b) node[midway, above, font=\tiny] {Encrypt$(m,pk_B)$};
  \draw[-{Latex[length=2mm]}, thick] (pk) -- (a);
  \draw[-{Latex[length=2mm]}, thick] (sk) -- (b);
  \node[font=\tiny, align=center] at (0,-1.5) {anyone may\\ hold $pk_B$};
  \node[font=\tiny, align=center] at (5,-3.35) {only Bob\\ ever holds $sk_B$};
\end{tikzpicture}
\\[2pt]
{\small \textbf{Asymmetric:} two different keys, only one ever secret.}
\end{minipage}
\caption{The structural difference in one picture. Symmetric encryption needs the same secret on both ends before anything can happen. Asymmetric encryption lets Alice encrypt using a key that was never secret at all --- only decryption needs the private half, and only Bob ever has it.}
\label{fig:symmetric-vs-asymmetric}
\end{figure}

\subsection{What Lattice-Based Crypto, RSA, and ECC Each Offer}
\label{sec:L2:what-lattice-rsa-ecc-each-offer}

All three of RSA, elliptic-curve cryptography (ECC), and lattice-based cryptography live on the asymmetric side --- but ``asymmetric cryptography'' isn't one single service. It covers (at least) three distinct jobs: \textbf{encryption} (hide a message so only the private-key holder reads it), \textbf{key exchange} (two parties agree on a shared secret over a public channel, with no prior relationship), and \textbf{digital signatures} (prove a message came from the private-key holder, without hiding anything). Not every family offers all three the same way.

\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|l|l|l|l|}
\hline
\textbf{Family} & \textbf{Encryption / KEM} & \textbf{Key exchange} & \textbf{Signatures} \\
\hline
...
\begin{rmkbox}{The Standard Fix: Hybrid Encryption}
Nearly every real protocol --- TLS (the padlock in your browser), SSH, Signal --- uses \textbf{both}, in a fixed order: run an asymmetric key exchange or KEM \emph{once}, briefly, to establish a shared symmetric key between two parties who have never met; then switch to fast symmetric encryption (typically AES or ChaCha20) for the actual data, using that newly shared key. Asymmetric crypto solves the one problem symmetric crypto structurally cannot (agreeing on a secret with no prior relationship); symmetric crypto then does the actual heavy lifting, fast. This is precisely why ML-KEM is called a \textbf{Key Encapsulation Mechanism} rather than an encryption scheme in its own right --- as you'll see in Lecture 3, its entire output is a short, fresh symmetric key, handed off to exactly this kind of hybrid handshake, not a general-purpose way to encrypt arbitrary messages.
\end{rmkbox}

\begin{figure}[ht]
\centering
\begin{tikzpicture}[scale=1, every node/.style={font=\small}]
  \node[draw, minimum width=8.4cm, minimum height=1.05cm, fill=orange!10, align=center] (phase1) at (0,2.4) {\textbf{Phase 1 --- Asymmetric (once, slow)}\\ \tiny key exchange / KEM: Alice and Bob, no prior shared secret, agree on session key $K$};
  \node[draw, minimum width=8.4cm, minimum height=1.05cm, fill=blue!8, align=center] (phase2) at (0,0.4) {\textbf{Phase 2 --- Symmetric (repeated, fast)}\\ \tiny AES/ChaCha20 under $K$: encrypt all the actual traffic, both directions};
  \draw[-{Latex[length=2mm]}, thick] (phase1) -- (phase2) node[midway, right, font=\tiny] {hand off $K$};
\end{tikzpicture}
\caption{The hybrid pattern nearly every real protocol follows: a single, expensive asymmetric step establishes a shared key, then cheap symmetric encryption handles everything after. Kyber's role in ML-KEM is entirely Phase 1 --- Lecture 3 picks this up in full.}
\label{fig:hybrid-handshake}
\end{figure}
...
\centering
\begin{tikzpicture}[scale=1, every node/.style={font=\small}]
  \node[draw, minimum width=3.1cm, minimum height=1cm, fill=gray!10, align=center] (svp) at (0,7.6) {\textbf{SVP}\\ \tiny find a short vector};
  \node[draw, minimum width=3.1cm, minimum height=1cm, fill=gray!10, align=center] (cvp) at (5.2,7.6) {\textbf{CVP}\\ \tiny find a nearby vector};

  \node[draw, minimum width=3.1cm, minimum height=1cm, fill=green!8, align=center] (sis) at (0,5.7) {\textbf{SIS}\\ \tiny same idea, random $A$};
  \node[draw, minimum width=3.1cm, minimum height=1cm, fill=blue!8, align=center] (lwe) at (5.2,5.7) {\textbf{LWE}\\ \tiny same idea, plus noise};

  \node[draw, minimum width=3.3cm, minimum height=1.3cm, text width=3.0cm, fill=orange!12, align=center] (trap) at (5.2,3.5) {\textbf{Trapdoor}\\ \tiny a secret ($\vec s$) lets the owner invert; no one else can};

  \node[draw, minimum width=3.1cm, minimum height=1cm, fill=green!8, align=center] (hash) at (0,1.5) {\textbf{Hash Functions}\\ \tiny no secret to hide};
  \node[draw, minimum width=3.1cm, minimum height=1cm, fill=blue!8, align=center] (enc) at (5.2,1.5) {\textbf{Encryption}\\ \tiny secret only owner recovers};

  \draw[-{Latex[length=2mm]}] (svp) -- (sis);
  \draw[-{Latex[length=2mm]}] (cvp) -- (lwe);
  \draw[-{Latex[length=2mm]}, green!35!black] (sis.south) -- (hash.north)
    node[midway, right, font=\tiny, text width=2.3cm, align=left, xshift=5pt] {no extra step needed};
  \draw[-{Latex[length=2mm]}, blue!45!black] (lwe.south) -- (trap.north);
  \draw[-{Latex[length=2mm]}, orange!70!black] (trap.south) -- (enc.north);

  \draw[dashed, gray!55] (-1.8,4.4) -- (7.0,4.4)
    node[midway, font=\tiny, gray!55, fill=white, inner sep=3pt] {both already one-way: same 3 requirements};
\end{tikzpicture}
\caption{SIS and LWE are not a detour from SVP and CVP --- they \emph{are} SVP and CVP, restricted to a random, easy-to-sample family of lattices built from a matrix $A$. SIS lives on the kernel lattice $\Lambda^\perp(A)$ (Section~\ref{sec:L2:the-short-integer-solution-sis-problem}) and keeps SVP's ``find a short vector'' shape; LWE lives on the image lattice $\Lambda_q(A)$ (Section~\ref{sec:L2:why-lwe-is-believed-hard-the-lattice-con}) and keeps CVP's ``find a nearby point'' shape. Both already satisfy the three requirements above the dashed line --- that alone makes each a plain one-way function. The paths then diverge: SIS's kernel/short-vector shape needs nothing further to give you ``no two things collide'' (hash functions --- nothing to hide), while LWE's image/nearby-point shape only becomes \emph{encryption} once a trapdoor is layered on top, giving one party (and only that party) an efficient inverse.}
\label{fig:svp-cvp-to-sis-lwe}
\end{figure}

\begin{rmkbox}{A Trapdoor Is a Fourth, Optional Ingredient --- Not Part of the Three Requirements}
The explicit \textbf{Trapdoor} box in Figure~\ref{fig:svp-cvp-to-sis-lwe} sits only on LWE's path, and that placement is the point. The three requirements from Section~\ref{sec:L2:one-way-functions-the-idea-behind-all-of} (a sampleable random instance, a worst-case reduction, an easy forward direction) are exactly what a plain one-way function needs, and that's already enough for \textbf{SIS-based hashing}: nobody --- not even whoever generated $A$ --- can efficiently find a collision. There is no secret held by any party, and none is needed; that's the entire point of a hash function, which is why SIS's path skips the trapdoor box entirely. A \textbf{trapdoor} is a separate, optional fourth ingredient, needed only when some designated party must be able to invert efficiently while everyone else can't: exactly what \textbf{LWE-based encryption} needs (the key-owner decrypts; nobody else can), and what Falcon's GPV sampling (Lecture 4) needs on the signing side.
\end{rmkbox}
